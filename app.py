...
# ── Inline Gradio queue primitive guard (works in Colab + Render) ─────────────
# patch_gradio.py patches queueing.py on disk for Render.
# This guard patches the Queue class in-memory so it works everywhere.
try:
    import asyncio as _aq_patch
    import gradio.queueing as _grq_patch
    import re as _re_patch, inspect as _ins_patch
    _q_src = _ins_patch.getsource(_grq_patch.Queue)
    _lock_a = set(_re_patch.findall(r"async with self\.(\w+)", _q_src))
    _evnt_a = set(_re_patch.findall(r"await self\.(\w+)\.(?:wait|set|clear)", _q_src))
    _orig_sp = _grq_patch.Queue.start_processing
    async def _safe_sp(self, *a, **kw):
        for _at in _lock_a:
            if getattr(self, _at, None) is None:
                setattr(self, _at, _aq_patch.Lock())
        for _at in _evnt_a:
            if getattr(self, _at, None) is None:
                setattr(self, _at, _aq_patch.Event())
        return await _orig_sp(self, *a, **kw)
    _grq_patch.Queue.start_processing = _safe_sp
    _orig_pu = _grq_patch.Queue.push
    async def _safe_pu(self, *a, **kw):
        for _at in _lock_a:
            if getattr(self, _at, None) is None:
                setattr(self, _at, _aq_patch.Lock())
        return await _orig_pu(self, *a, **kw)
    _grq_patch.Queue.push = _safe_pu
except Exception:
    pass
# ──────────────────────────────────────────────────────────────────────────────
...
COL_REMAP_FIXED = {
    ...
    # ── DataNetra realistic MSME diagnostic dataset columns ──────────────────
    "Revenue":                 "Monthly_Sales_INR",
    "COGS":                    "Monthly_Operating_Cost_INR",
    "Gross_Margin_Pct":        "Avg_Margin_Percent",
    "SKU":                     "SKU_Name",
    "Category":                "Product_Category",
    # ── Additional common real-world column name variants ─────────────────
    # Sales / Revenue
    "Sales":                   "Monthly_Sales_INR",
    "sale_amount":             "Monthly_Sales_INR",
    "Net_Sales":               "Monthly_Sales_INR",
    "Total_Sales":             "Monthly_Sales_INR",
    "Sales_Amount":            "Monthly_Sales_INR",
    "sales_value":             "Monthly_Sales_INR",
    "total_revenue":           "Monthly_Sales_INR",
    "Total_Revenue":           "Monthly_Sales_INR",
    "Turnover":                "Monthly_Sales_INR",
    "sales":                   "Monthly_Sales_INR",
    "revenue":                 "Monthly_Sales_INR",
    "income":                  "Monthly_Sales_INR",
    "Income":                  "Monthly_Sales_INR",
    # Units / Demand
    "Sales_Quantity":          "Monthly_Demand_Units",
    "Qty_Sold":                "Monthly_Demand_Units",
    "qty_sold":                "Monthly_Demand_Units",
    "Units_Sold":              "Monthly_Demand_Units",
    "Quantity":                "Monthly_Demand_Units",
    "quantity":                "Monthly_Demand_Units",
    "sales_qty":               "Monthly_Demand_Units",
    "Sales_Qty":               "Monthly_Demand_Units",
    # Cost / COGS
    "cost":                    "Monthly_Operating_Cost_INR",
    "Cost":                    "Monthly_Operating_Cost_INR",
    "Cost_of_Goods":           "Monthly_Operating_Cost_INR",
    "cost_of_goods":           "Monthly_Operating_Cost_INR",
    "COGS_INR":                "Monthly_Operating_Cost_INR",
    "Total_Cost":              "Monthly_Operating_Cost_INR",
    "total_cost":              "Monthly_Operating_Cost_INR",
    "Purchase_Cost":           "Monthly_Operating_Cost_INR",
    # Margin
    "margin":                  "Avg_Margin_Percent",
    "Margin":                  "Avg_Margin_Percent",
    "margin_pct":              "Avg_Margin_Percent",
    "Margin_Pct":              "Avg_Margin_Percent",
    "Margin_Percent":          "Avg_Margin_Percent",
    "Gross_Margin":            "Avg_Margin_Percent",
    "gross_margin":            "Avg_Margin_Percent",
    "Net_Margin":              "Avg_Margin_Percent",
    "Profit_Margin":           "Avg_Margin_Percent",
    "profit_margin":           "Avg_Margin_Percent",
    # Returns
    "return_units":            "Returns_Percentage",
    "Return_Units":            "Returns_Percentage",
    "returns_qty":             "Returns_Percentage",
    "Returns_Qty":             "Returns_Percentage",
    "Return_Rate":             "Returns_Percentage",
    "return_rate":             "Returns_Percentage",
    "Refund_Rate":             "Returns_Percentage",
    # Product / SKU
    "product_name":            "SKU_Name",
    "item_name":               "SKU_Name",
    "Item_Name":               "SKU_Name",
...
def _apply_col_remap(df):
    ...
    # ── Derive Returns_Percentage from Returns_Units if not yet present ──────
    # Dataset may carry Returns_Units (integer count) instead of a % column.
    # Convert: pct = returns_units / sales_quantity * 100, clipped to [0, 100].
    if "Returns_Percentage" not in df.columns:
        _ru_col = next((c for c in df.columns if c in ("Returns_Units", "Return_Units", "Returns_Qty", "return_units", "returns_qty", "Returns_Qty")), None)
        _sq_col = next((c for c in df.columns if c in ("Monthly_Demand_Units", "Sales_Quantity")), None)
        if _ru_col and _sq_col:
            df["Returns_Percentage"] = (
                df[_ru_col] / df[_sq_col].replace(0, 1) * 100
            ).clip(0, 100)
        elif _ru_col:
            df["Returns_Percentage"] = df[_ru_col].clip(0, 100)

    # ── Derive Monthly_Sales_INR from qty × Unit_Price if missing ──────────
    # Handles datasets that carry line-level price/quantity instead of aggregated revenue.
    # Uses Monthly_Demand_Units (post-remap canonical) or any surviving qty column.
    if "Monthly_Sales_INR" not in df.columns:
        _qty_col = next((c for c in df.columns if c in ("Monthly_Demand_Units", "Sales_Quantity", "Quantity", "qty_sold", "Sales_Qty")), None)
        if _qty_col and "Unit_Price" in df.columns:
            df["Monthly_Sales_INR"] = (df[_qty_col] * df["Unit_Price"]).clip(lower=0)

    # ── Derive Monthly_Operating_Cost_INR from qty × Unit_Cost ─────────────
    if "Monthly_Operating_Cost_INR" not in df.columns:
        _qty_col = next((c for c in df.columns if c in ("Monthly_Demand_Units", "Sales_Quantity", "Quantity", "qty_sold", "Sales_Qty")), None)
        if _qty_col and "Unit_Cost" in df.columns:
            df["Monthly_Operating_Cost_INR"] = (df[_qty_col] * df["Unit_Cost"]).clip(lower=0)

    # ── Derive Avg_Margin_Percent from revenue/cost if still missing ──────────
    if "Avg_Margin_Percent" not in df.columns:
        if "Monthly_Sales_INR" in df.columns and "Monthly_Operating_Cost_INR" in df.columns:
            _rev = df["Monthly_Sales_INR"].replace(0, float("nan"))
            df["Avg_Margin_Percent"] = (
                (_rev - df["Monthly_Operating_Cost_INR"]) / _rev * 100
            ).clip(0, 100).fillna(0)
    ...
    if "Inventory_Turnover" not in df.columns:
        ...
        elif "inventory_level" in df.columns and "Monthly_Demand_Units" in df.columns:
            df["Inventory_Turnover"] = (
                df["Monthly_Demand_Units"] / df["inventory_level"].replace(0, 1)
            ).clip(0, 12)
    ...
    # Derive Monthly_Operating_Cost_INR if missing (final fallback: 60% of revenue)
    if "Monthly_Operating_Cost_INR" not in df.columns:
...
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  DATA SAFETY LAYER  —  Raw dataset cleaning before analytics pipeline      ║
# ║  Cleans messy MSME values before _apply_col_remap or calculate_scores      ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

# Columns that must be numeric — covers pre-remap names AND canonical names
_NUMERIC_CLEAN_COLS = {
    # Pre-remap / raw names that arrive from user datasets
    'Sales_Quantity', 'Unit_Price', 'Unit_Cost',
    'Revenue', 'COGS', 'Gross_Margin_Pct',
    'Returns_Units', 'Return_Units', 'Replacement_Units',
    'Inventory_Level', 'Opening_Inventory', 'Reorder_Point',
    'Supplier_Lead_Time_Days', 'Payment_Delay_Days',
    'Customer_Count', 'Promotion_Flag', 'Stockout_Flag',
    # Canonical names (post-remap)
    'Monthly_Sales_INR', 'Monthly_Operating_Cost_INR',
    'Outstanding_Loan_INR', 'Avg_Margin_Percent',
    'Monthly_Demand_Units', 'Returns_Percentage',
    'Vendor_Delivery_Reliability', 'Inventory_Turnover',
...
def _clean_raw_df(df):
    ...
    before _apply_col_remap and before any analytics.

    What it does:
      1. Strip currency symbols (₹ $ £ €), commas, and whitespace from numeric fields.
      2. Convert to float with errors='coerce', fill NaN → 0.
      3. Parse Date column with errors='coerce', drop rows where date is NaT.
      4. Clip computed ratios (Margin %, Returns %, Inventory Turnover) to safe ranges
         so divide-by-zero never reaches the scoring or forecasting layer.

    What it does NOT do:
      - Does not rename columns (that is _apply_col_remap's job).
      - Does not add missing columns (that is calculate_scores' job).
    ...
    _cols_to_clean = [c for c in df.columns if c in _NUMERIC_CLEAN_COLS]

    # Also clean any column whose name ends with common numeric suffixes
    _numeric_suffixes = ('_INR', '_Units', '_Pct', '_Rate', '_Score',
                         '_Count', '_Days', '_Level', '_Flag', '_Qty',
                         '_Price', '_Cost', '_Revenue', '_Percent')
    for c in df.columns:
        if c not in _cols_to_clean and c.endswith(_numeric_suffixes):
            _cols_to_clean.append(c)

    for col in _cols_to_clean:
        if col not in df.columns:
            continue
        try:
            s = df[col]
            # If already a clean numeric dtype, just fill NaN and move on
            if pd.api.types.is_numeric_dtype(s):
                df[col] = s.fillna(0)
                continue
            # Clean string/object/mixed columns — handles ₹, commas, NA text, etc.
            _NULL_WORDS = {'na', 'n/a', 'nil', 'none', 'null', '-', '', 'nan'}
            def _scrub(v):
                v = str(v).strip()
                for ch in ('₹', '$', '£', '€', ',', ' '):
                    v = v.replace(ch, '')
            ...
            df[col] = pd.to_numeric(s.map(_scrub), errors='coerce').fillna(0)
        except Exception:
            pass   # never let cleaning break ingestion — skip silently

    # ── 2. Date parsing ───────────────────────────────────────────────────────
    # Try both 'Date' and common pre-remap date column names
    _date_col_candidates = [c for c in df.columns
                             if c.lower() in ('date', 'transaction_date', 'sales_date',
                                              'month', 'period', 'order_date')]
    for _dc in _date_col_candidates:
        try:
            df[_dc] = pd.to_datetime(df[_dc], errors='coerce', dayfirst=False)
            # Drop rows with completely unparseable dates only when >50% are valid
            # (avoids nuking the whole dataset for one bad column)
            _valid_ratio = df[_dc].notna().mean()
            if _valid_ratio >= 0.5:
                df = df.dropna(subset=[_dc])
        except Exception:
            pass
    ...
    if 'Avg_Margin_Percent' in df.columns:
        df['Avg_Margin_Percent'] = pd.to_numeric(
            df['Avg_Margin_Percent'], errors='coerce').fillna(0).clip(0, 100)

    # Clip Returns_Percentage: valid range 0–100 %
    if 'Returns_Percentage' in df.columns:
        df['Returns_Percentage'] = pd.to_numeric(
            df['Returns_Percentage'], errors='coerce').fillna(0).clip(0, 100)

    # Clip Gross_Margin_Pct (pre-remap name): 0–100
    if 'Gross_Margin_Pct' in df.columns:
        df['Gross_Margin_Pct'] = pd.to_numeric(
            df['Gross_Margin_Pct'], errors='coerce').fillna(0).clip(0, 100)

    # Clip Inventory_Turnover: 0–12× per month (capped in _apply_col_remap too, belt+braces)
    if 'Inventory_Turnover' in df.columns:
        df['Inventory_Turnover'] = pd.to_numeric(
            df['Inventory_Turnover'], errors='coerce').fillna(0).clip(0, 12)

    # Protect Unit_Price / Unit_Cost from being zero before qty×price derivation
    for _price_col in ('Unit_Price', 'Unit_Cost'):
        if _price_col in df.columns:
            df[_price_col] = pd.to_numeric(
                df[_price_col], errors='coerce').fillna(0).clip(lower=0)
...
def generate_granular_forecast(df):
    ...
    def _best_forecast(monthly_df, n_periods=12):
        ...
        # ── A: Prophet — SKIPPED in granular forecasts ──────────────────────
        # Prophet requires 3-8s per fit × 2 fits × 16 entities = up to 256s hang.
        # The main forecast_sales() already runs Prophet for the overall summary.
        # Granular per-entity forecasts use fast models only (HW + LR + Baseline).
        prophet_fc = None  # not used in granular path
...
SNP_CATALOG = {
    'GeM (Government e-Marketplace)': {
        ...
        'ondc_categories':['Industrial Supplies','Electronics','Stationery & Books',
                           'Health & Pharma','Agricultural Inputs','Services'],
    ...
    'Flipkart Commerce (ONDC)': {
        ...
        'ondc_categories':['Grocery & Staples','Snacks & Branded Food','Beverages',
                           'Personal Care','Home Care','Electronics','Apparel & Clothing',
                           'Footwear','Home & Kitchen','Dairy & Eggs','Fruits & Vegetables'],
    ...
    'Meesho (ONDC)': {
        ...
        'ondc_categories':['Apparel & Clothing','Footwear','Personal Care',
                           'Home & Kitchen','Snacks & Branded Food','Grocery & Staples'],
    ...
    'NSIC e-Marketplace': {
        ...
        'ondc_categories':['Industrial Supplies','Agricultural Inputs','Electronics',
                           'Health & Pharma','Stationery & Books','Services'],
        'min_health':25,'segment_boost':['Champions','Loyal','Potential'],
        'channel_type':'b2b_institutional',
        'b2c_fit': [],
    ...
    'Amazon Seller Services (ONDC)': {
        ...
        'ondc_categories':['Electronics','Personal Care','Health & Pharma',
                           'Apparel & Clothing','Home & Kitchen','Grocery & Staples',
                           'Snacks & Branded Food','Footwear'],
        'min_health':35,'segment_boost':['Champions','Loyal'],
        'channel_type':'b2c_marketplace',
        'b2c_fit': ['Electronics','FMCG','Clothing','Supermarket','Hypermarket'],
        'margin_sweet_spot': (18, 40),
    ...
    'Udaan (B2B ONDC)': {
        ...
        'ondc_categories':['Grocery & Staples','Snacks & Branded Food','Beverages',
                           'Apparel & Clothing','Dairy & Eggs','Fruits & Vegetables',
...
def _extract_business_name(df, user_data):
    """
    Return the business display name, always from the dataset first.

    Priority:
      1. Explicit name column in dataset:
         Business_Name / Company / MSME_Name / Enterprise_Name /
         Organisation_Name / Firm_Name / Brand_Name / Shop_Name
      2. MSME_ID / Udyam_Number column value (the raw ID from the dataset)
      3. user_data['company_name'] — only when no dataset is available (PDF etc.)
      4. "MSME Business Dataset"

    No registry lookup — the dataset IS the source of truth.
    Whatever is in the dataset columns is shown exactly as-is.
    """
    _NAME_COLS = (
    ...
    _ID_COLS = (
        'MSME_ID', 'msme_id',
        'Udyam_Number', 'udyam_number', 'UDYAM_NUMBER',
        'Udyam_No',     'udyam_no',
        'msme_number',  'MSME_Number',
    )
    _SKIP = frozenset({
        '', '-', '—', 'na', 'n/a', 'none', 'null', 'nan',
        'default', 'your company', 'msme business dataset',
    })

    def _pick(series):
        """First non-blank value from a Series."""
        try:
            vals = series.dropna().astype(str).str.strip()
            vals = vals[vals.str.lower().map(lambda v: v not in _SKIP)]
            return vals.iloc[0] if len(vals) else None
        except Exception:
            return None

    if df is not None:
        # Priority 1 — explicit business name column
        ...
        # Priority 2 — MSME_ID / Udyam column (raw value, no lookup)
        for col in _ID_COLS:
            if col in df.columns:
                val = _pick(df[col])
                if val:
    ...
    # Priority 3 — user_data (only when no dataset available, e.g. PDF cover)
    ud_name = ((user_data or {}).get('company_name') or '').strip()
    if ud_name and ud_name.lower() not in _SKIP:
        return ud_name

    return "MSME Business Dataset"
...
def generate_insights(user_data, df_raw, lang='en'):
    ...
    try:
        ...
        company = _extract_business_name(df, user_data)
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:6px">
  ...
  <div style="background:rgba(255,255,255,0.06);border-radius:6px;padding:7px 11px">
    ...
    <div style="font-size:10px;color:#A8C8E8;margin-top:1px">{total_records:,} records · {total_products:,} SKU{'s' if total_products!=1 else ''}</div>
        ...
        # Compact preprocessing validation block — appears before Section 1
        _total_rows_raw  = len(df) if df is not None else 0
        _valid_rows      = int(df.dropna(subset=['Monthly_Sales_INR']).shape[0]) if df is not None and 'Monthly_Sales_INR' in df.columns else _total_rows_raw
        _invalid_removed = max(0, _total_rows_raw - _valid_rows)
        _date_status     = "✅ Date column detected and parsed" if 'Date' in (df.columns if df is not None else []) else "⚠️ No date column — time-series analysis limited"
        _cat_status      = "✅ Product and category mapping verified" if 'Product_Category' in (df.columns if df is not None else []) else "ℹ️ No category column — category analysis limited"
        _currency_status = "✅ Currency values normalised (₹ symbols and commas cleaned)"
        _missing_status  = "✅ Missing values handled — filled with dataset defaults"
        _invalid_status  = (f"✅ {_invalid_removed} invalid record(s) removed" if _invalid_removed > 0
                            else "✅ No invalid records detected")
        html += f"""<div style="margin:0 48px 0;padding:14px 20px 12px;
background:#F4F9FF;border:1px solid #D0E4F8;border-radius:10px;border-left:3px solid #1B4F8A">
  <div style="font-size:9px;font-weight:800;letter-spacing:1.8px;text-transform:uppercase;
              color:#1B4F8A;margin-bottom:8px">📋 Data Analysis Summary</div>
  <div style="font-size:10px;color:#4A6A8A;line-height:1.5;font-style:italic;margin-bottom:8px">
    Summary of dataset validation and preprocessing completed before analytics.
  </div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 24px">
    <div style="font-size:11px;color:#1B4F8A">{_currency_status}</div>
    <div style="font-size:11px;color:#1B4F8A">{_missing_status}</div>
    <div style="font-size:11px;color:#1B4F8A">{_invalid_status}</div>
    <div style="font-size:11px;color:#1B4F8A">{_cat_status}</div>
    <div style="font-size:11px;color:#1B4F8A">{_date_status}</div>
    <div style="font-size:11px;color:#1B4F8A">✅ Dataset validated for business analysis</div>
  </div>
</div>"""

        # Section 1 — KPIs
        html += _sb_divider(1, 'Retail Business Performance', 'Retail Business Performance Overview')
...
{_kpi_card(prf_acc, '⭐', 'Performance Score', f'{perf_score:.1f}%', f'<span class="sb-status-badge {_health_cls(perf_score)}" style="margin-top:8px">{prf_lbl}</span>')}
        ...
        # Section 2 — Scores
        html += _sb_divider(2, 'Health & Operational Scores', 'AI Business Health & Operational Scores')
...
{_score_card_full('🌐', 'ONDC Readiness Score', f'{ondc_readiness:.0f}%', ondc_readiness/100,
    ...
    _ors_cls, _ors_lbl)}
        ...
        # Forecast
        html += _sb_divider(4, 'AI Revenue Projection', 'AI Revenue Projection – Baseline Business Forecast')
...
<div style="margin:12px 48px 0;padding:14px 22px;background:linear-gradient(90deg,#EBF4FF,#F5F0FF);border-radius:10px;border-left:4px solid #1B4F8A;display:grid;grid-template-columns:repeat(4,1fr);gap:0;align-items:center;">
  ...
  <div style="padding:10px 16px;background:#F0F7FF;font-size:11px;color:#4A6A8A">
    All 4 models run on your data. Each is validated on held-out months using MAE (Mean Absolute Error) — the model with lowest MAE wins and produces the final forecast. Lower MAE = closer to actual sales. Holt-Winters runs in pure numpy — no installation needed.
        ...
        avg_ret   = float(df[ret_col].mean()) if ret_col else 0.0
        ...
        def _persona_fit_score(p):
            ...
            if avg_ret <= p['ret_max']:       score += 40
            elif avg_ret <= p['ret_max']*1.5: score += 20
            # Margin check (25 pts)
...
<thead><tr style="background:#0B1F3A">
  ...
  <th style="padding:11px 14px;text-align:center;color:#A8D8FF;font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase">Product Return Rate</th>
...
<div style="margin-top:8px;font-size:11px;color:#4A6A8A">✅ &lt;4% return rate · ⚠️ 4–7% · 🔴 &gt;7%</div></div>"""
        ...
        # ── Pillar 2 → SNP: collect ONDC categories from classified data ────
        _ondc_cats_for_snp = set()
        if df_raw is not None and 'ONDC_Category' in df_raw.columns:
            _ondc_cats_for_snp = set(df_raw['ONDC_Category'].dropna().unique().tolist())

        snp_scores = {}
        for snp_name, snp_data in SNP_CATALOG.items():
            s = 0.0

            # ── SIGNAL 1: ONDC Category overlap — 40 pts (PRIMARY, data-driven) ──
            # Uses actual SKU categories from Pillar 2 classification.
            # Each matching category = 5 pts, capped at 40.
            # Falls back to business type match if no ONDC categories available.
            _snp_ondc_cats = set(snp_data.get('ondc_categories', []))
            if _ondc_cats_for_snp and _snp_ondc_cats:
                _cat_overlap = len(_ondc_cats_for_snp & _snp_ondc_cats)
                _cat_total   = max(len(_snp_ondc_cats), 1)
                # Proportional: full overlap = 40 pts, partial overlap scales down
                _cat_score   = min(40, round((_cat_overlap / _cat_total) * 40 + _cat_overlap * 3))
                s += _cat_score
            else:
                # Fallback: business type dropdown match (used only when no ONDC data)
                if biz_type in snp_data['business_types']:
                    s += 35
                elif any(g in biz_type for g in snp_data.get('business_types', [])):
                    s += 15

            # ── SIGNAL 1b: Channel-type fit — 8 pts ──────────────────────────────
            # B2C products → B2C marketplace; B2B products → wholesale/institutional
            _ch_type = snp_data.get('channel_type', '')
            _b2c_fit = snp_data.get('b2c_fit', [])
            # Detect B2C vs B2B from ONDC categories when available
            _b2c_ondc = {'Grocery & Staples','Snacks & Branded Food','Personal Care',
                         'Home Care','Apparel & Clothing','Footwear','Electronics',
                         'Beverages','Dairy & Eggs','Fruits & Vegetables','Home & Kitchen'}
            _b2b_ondc = {'Industrial Supplies','Agricultural Inputs','Services'}
            if _ondc_cats_for_snp:
                _has_b2c = bool(_ondc_cats_for_snp & _b2c_ondc)
                _has_b2b = bool(_ondc_cats_for_snp & _b2b_ondc)
            else:
                _has_b2c = _is_b2c_retail
                _has_b2b = not _is_b2c_retail
            if _has_b2c and _ch_type == 'b2c_marketplace':
                s += 8
            elif _has_b2c and _ch_type == 'social_commerce':
                s += 5
            elif _has_b2b and _ch_type in ('b2b_wholesale', 'b2b_institutional'):
                s += 8
            elif _has_b2b and _ch_type == 'government':
                s += 6
        ...
        html += _sb_divider(5, 'SNP Matching', 'Intelligent SNP Matching (Data-Driven)')
        ...
        _opp_html  = _sb_divider(3, 'Risk & Profitability Insights', 'Operational Risk & Profitability Insights')
...
<div class="sb-opp-card">
  ...
  <div class="sb-opp-body">
    ...
    <div class="sb-opp-metric-row">
      <span class="sb-opp-metric-label">Platform Fit Index</span>
      <span class="sb-opp-metric-value" style="color:{_mkt_colour}">{_mkt_top_score}/100</span>
    </div>
    <div class="sb-opp-metric-row">
  ...
  <div style="background:#FFFFFF;border:1px solid #C8DCEF;border-radius:12px;padding:18px 20px;text-align:center;border-top:3px solid #B0BEC5">
    <div style="font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#4A6A8A;margin-bottom:8px">📊 Current Retail Revenue (Baseline)</div>
    <div style="font-size:26px;font-weight:900;color:#7A92AA;font-family:monospace">{_fmt_inr_sb(rev_before_ondc)}</div>
    <div style="font-size:11px;color:#4A6A8A;margin-top:4px">Your estimated retail revenue before ONDC marketplace activation.</div>
  </div>
  <div style="background:#FFFFFF;border:1px solid #C8DCEF;border-radius:12px;padding:18px 20px;text-align:center;border-top:3px solid #1a7a40">
    <div style="font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#4A6A8A;margin-bottom:8px">🚀 Projected Revenue with ONDC</div>
    <div style="font-size:26px;font-weight:900;color:#1a7a40;font-family:monospace">{_fmt_inr_sb(total_sales)}</div>
    <div style="font-size:11px;color:#4A6A8A;margin-top:4px">Projected total revenue including ONDC marketplace uplift.</div>
  </div>
  <div style="background:#EAF7EE;border:1px solid #C3E6CB;border-radius:12px;padding:18px 20px;text-align:center;border-top:3px solid #1a7a40">
    <div style="font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#1a7a40;margin-bottom:8px">📈 Estimated ONDC Revenue Increase</div>
  ...
  <div style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#4A6A8A;margin-bottom:14px">💰 Revenue Simulation — ONDC Platform Projection</div>
  {plat_attr_rows}
  ...
  <div style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#4A6A8A;margin-bottom:10px">⭐ Top Products for ONDC Marketplace Expansion</div>
  <table style="width:100%;border-collapse:collapse;background:#FFFFFF;border-radius:12px;overflow:hidden;border:1px solid #C8DCEF;font-size:13px">
    <thead><tr style="background:#0B1F3A">
      ...
      <th style="padding:10px 14px;text-align:left;color:#A8D8FF;font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase">Product Return Rate</th>
    </tr></thead>
    <tbody>{prod_rows}</tbody>
  </table>
  <div style="font-size:11px;color:#4A6A8A;margin-top:6px">Products ranked by marketplace suitability: sales performance, margin strength, and return rate.</div>
</div>"""

        # SNP Fit Score cards (single clean card grid — no duplicate list)
        html += f"""<div style="margin:20px 48px 0">
  <div style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#4A6A8A;margin-bottom:14px">🏪 Recommended ONDC Marketplace Platforms</div>
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px">
    <span style="font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:#4A6A8A">Platform Fit Index</span>
    <span style="font-size:20px;font-weight:900;color:{bar_col}">{score}<span style="font-size:11px;font-weight:500;color:#4A6A8A">/100</span></span>
  </div>
  ...
  <div style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#4A6A8A;margin-bottom:4px">📦 Product Category Intelligence</div>
  <div style="font-size:12px;color:#6A8AA8;margin-bottom:10px;font-style:italic">Category-level revenue distribution and performance signals across your product catalogue.</div>
  {cat_summary_html if cat_summary_html else '<div style="background:#FFFFFF;border-radius:10px;padding:14px;color:#4A6A8A;font-style:italic;border:1px solid #C8DCEF">No category data available in dataset.</div>'}
        ...
        if _bot_cat_name != '—' and _bot_cat_name != _top_cat_name:
            _exec_parts.append(f'margin pressure has been identified in the {_s(_bot_cat_name)} category — supplier renegotiation or pricing review is recommended')
        if _stock_6w_risk:
            _exec_parts.append(f'a {_s("stock shortfall")} is projected within the next six weeks — advance procurement planning is recommended to avoid fulfilment disruption')
        if vendor_sc < 65:
...
border-radius:14px;border-left:5px solid #FFD080;box-shadow:0 4px 18px rgba(11,31,58,0.18)">
  ...
  <div style="font-size:13px;color:#FFFFFF;line-height:1.8;font-weight:400">{_summary_bullets_html}</div>
        ...
        # Recommendations
        html += _sb_divider(6, 'Business Actions', 'Recommended Business Actions')
        ...
        if avg_ret > 7:
            ...
            _imm_rows.append(('reco-medium', 'Medium',
                f'Return rate is healthy at <strong>{avg_ret:.1f}%</strong> — '
        ...
        if health_score >= 65:
            _str_rows.append(('reco-medium', 'Medium',
                ...
                f'complete SNP integration with <strong>{top_snp_name}</strong> (fit index: {top_snp_score}/100).'))
        else:
            _str_rows.append(('reco-medium', 'Medium',
...
def generate_dashboard_data(user_data, df):
    try:
        ...
        total_products = df[sku_col].nunique() if sku_col else 0; company_name = _extract_business_name(df, user_data)
        # ──────────────────────────────────────────────────────────────────────
        # ONDC Journey KPI Panel  (health/perf scores removed — shown in AI insights)
        # ──────────────────────────────────────────────────────────────────────
        company_name = _extract_business_name(df, user_data)
        msme_key     = user_data.get('msme_number',  '—')
        biz_type     = user_data.get('business_type', 'FMCG')

        # Helper: resolve column — first name found in df wins (variadic, any number of aliases)
        def _scol(*names):
            for n in names:
        ...
        # ── ONDC Data Detection ───────────────────────────────────────────────
        # Check if any ONDC operational column is present in the uploaded dataset.
        # Also try to infer ONDC revenue from a channel/platform column if present.
        _ONDC_COL_SIGNALS = {
            'ondc_revenue', 'ondc_channel_revenue', 'revenue_before_ondc',
            'revenue_after_ondc', 'ondc_flag', 'ondc_status', 'ondc_enabled',
            'sales_channel', 'channel', 'order_source', 'platform',
            'marketplace', 'marketplace_name', 'platform_name',
            'units_returned', 'replacement_units', 'replacement_count',
            'quantity_returned', 'rolling_6m_return_rate',
            'target_achievement_pct', 'fulfillment_rate',
        }
        _df_cols_lower = {c.lower(): c for c in df.columns}
        _ondc_cols_found = [orig for low, orig in _df_cols_lower.items()
                            if low in _ONDC_COL_SIGNALS]
        ondc_data_available = len(_ondc_cols_found) > 0

        # ── ONDC row detection: channel/flag column → boolean mask ─────────────
        # Step 1: look for Sales_Channel / Channel / Platform col containing "ONDC"
        # Step 2: look for ONDC_Flag col with value "Yes" / 1 / True
        # Step 3: fall back to explicit boc / ochc columns if present
        _channel_col = next(
            (orig for low, orig in _df_cols_lower.items()
             if low in ('sales_channel', 'channel', 'platform', 'marketplace',
                        'order_source', 'marketplace_name', 'platform_name')),
            None
        )
        _flag_col = next(
            (orig for low, orig in _df_cols_lower.items()
             if low in ('ondc_flag', 'ondc_status', 'ondc_enabled', 'is_ondc')),
            None
        )

        # Build a boolean mask of ONDC rows
        _ondc_mask = None
        if _channel_col and gc:
            try:
                _ch_mask = df[_channel_col].astype(str).str.upper().str.contains('ONDC', na=False)
                if _ch_mask.any():
                    _ondc_mask = _ch_mask
                    ondc_data_available = True
            except Exception:
                pass
        if _ondc_mask is None and _flag_col and gc:
            try:
                _fl_mask = df[_flag_col].astype(str).str.lower().isin(['yes', '1', 'true', 'y', 'ondc'])
                if _fl_mask.any():
                    _ondc_mask = _fl_mask
                    ondc_data_available = True
            except Exception:
        ...
        # Derive synthetic boc / ochc from mask if dedicated columns absent
        if _ondc_mask is not None and gc:
            df = df.copy()
            df['_ondc_row']         = _ondc_mask.astype(float)          # 1 = ONDC row
            df['_ondc_rev_inferred']= df[gc] * _ondc_mask.astype(float) # revenue on ONDC rows
            df['_nondc_rev_inferred']= df[gc] * (~_ondc_mask).astype(float) # non-ONDC revenue
            if not ochc:
                ochc = '_ondc_rev_inferred'
            if not boc:
                boc = '_nondc_rev_inferred'

        # ── ONDC Active Since: earliest date among ONDC rows ─────────────────
        _ondc_active_since = "Not available"
        if _ondc_mask is not None and dc and dc in df.columns:
            try:
                _ondc_dates = pd.to_datetime(
                    df.loc[_ondc_mask, dc], errors='coerce'
                ).dropna()
                if len(_ondc_dates) > 0:
                    _earliest = _ondc_dates.min()
                    _ondc_active_since = _earliest.strftime('%b %Y')   # e.g. "Mar 2024"
            except Exception:
                pass
        elif ochc and dc and dc in df.columns:
            # Fallback: explicit ondc_channel_revenue column — find first non-zero row
            try:
                _rev_col = ochc if ochc in df.columns else None
                if _rev_col:
                    _ondc_rev_mask = pd.to_numeric(df[_rev_col], errors='coerce').fillna(0) > 0
                    _ondc_dates2   = pd.to_datetime(
                        df.loc[_ondc_rev_mask, dc], errors='coerce'
                    ).dropna()
                    if len(_ondc_dates2) > 0:
                        _ondc_active_since = _ondc_dates2.min().strftime('%b %Y')
            except Exception:
                pass
        ...
        # ondc_pos: sum of ONDC-attributed revenue (clip negative to 0)
    ...
    <table style="width:100%;border-collapse:collapse;font-size:12px;border:1px solid #D0E4F4;border-radius:0 0 8px 8px;overflow:hidden">
      <thead>
        <tr style="background:#0B1F3A">
          ...
          <th style="padding:9px 14px;text-align:right;color:#A8D8FF;font-size:11px;font-weight:700">Total Revenue</th>
          <th style="padding:9px 14px;text-align:right;color:#A8D8FF;font-size:11px;font-weight:700">Baseline Retail Revenue</th>
          <th style="padding:9px 14px;text-align:right;color:#A8D8FF;font-size:11px;font-weight:700">ONDC Marketplace Revenue</th>
          <th style="padding:9px 14px;text-align:center;color:#A8D8FF;font-size:11px;font-weight:700">Uplift</th>
          <th style="padding:9px 14px;text-align:center;color:#A8D8FF;font-size:11px;font-weight:700">Product Return Rate</th>
      ...
      <tbody>{rows}</tbody>
        ...
        # ── "No ONDC data" banner (shown only when ondc_data_available is False) ──
        _no_ondc_banner = "" if ondc_data_available else """
  <div style="background:linear-gradient(135deg,#FFF8E7,#FFFBF0);border:1.5px solid #F39C12;
              border-left:5px solid #F39C12;border-radius:10px;padding:16px 20px;margin-bottom:18px">
    <div style="display:flex;align-items:flex-start;gap:12px">
      <span style="font-size:1.8rem;line-height:1">ℹ️</span>
      <div>
        <div style="font-size:13px;font-weight:800;color:#7D4E00;margin-bottom:6px">
          ONDC Operational Data Not Detected
        </div>
        <div style="font-size:12px;color:#5C3900;line-height:1.65;margin-bottom:8px">
          ONDC operational data was not found in the uploaded dataset.
          This dashboard becomes fully active when ONDC-specific fields such as
          <strong>sales channel, marketplace/platform, ONDC revenue, returns,</strong> or
          <strong>replacements</strong> are available.
        </div>
        <div style="font-size:11.5px;color:#7D4E00;background:#FEF3CD;border-radius:6px;
                    padding:8px 12px;border:1px solid #F9D67A">
          💡 You can still use <strong>Step 5</strong> for ONDC readiness scoring, sales forecasting,
          and marketplace opportunity analysis. The charts below show placeholders until
          ONDC data is uploaded.
        </div>
      </div>
    </div>
  </div>"""
  ...
  <div style="background:linear-gradient(135deg,#0B1F3A,#1B4F8A);border-radius:10px;padding:16px 24px;
    ...
    <div>
      <div style="color:#FFFFFF;font-size:1.15rem;font-weight:800;letter-spacing:0.3px">ONDC Business Impact &amp; Marketplace Performance</div>
      <div style="color:#A8D8FF;font-size:0.84rem;margin-top:3px">Retail performance analysis highlighting ONDC revenue uplift, operational impact, and store-level marketplace performance. KPI totals reflect the full 24-month dataset; health and risk scores are computed on the trailing 12 months.</div>
    </div>
    <div style="margin-left:auto;text-align:right">
      <div style="color:#A8D8FF;font-size:10px;letter-spacing:1px;text-transform:uppercase">ONDC Status</div>
      {'<div style="color:#52e88a;font-size:13px;font-weight:800">● Active</div>' if ondc_data_available and _ondc_active_since != "Not available" else
       '<div style="color:#f39c12;font-size:13px;font-weight:800">● Pre-Activation</div>'}
      <div style="color:#A8D8FF;font-size:10px;margin-top:5px;letter-spacing:0.5px">
        ONDC Active Since: <strong style="color:#FFFFFF">{_ondc_active_since}</strong>
      </div>
    </div>
  </div>
  {_no_ondc_banner}
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px">
    <div style="background:#FFFFFF;border:1px solid #D0E4F4;border-radius:10px;padding:14px 12px;border-top:3px solid #1B4F8A;text-align:center">
      <div style="font-size:9px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;color:#7A92AA;margin-bottom:6px">Total Business Revenue</div>
      <div style="font-size:18px;font-weight:900;color:#0B1F3A;font-family:monospace">{fmt_inr(total_gross)}</div>
      <div style="font-size:10px;color:#7A92AA;margin-top:3px">{total_qty:,} units sold across all channels</div>
    </div>
    {'<div style="background:#FFFFFF;border:1px solid #C3E6CB;border-radius:10px;padding:14px 12px;border-top:3px solid #27ae60;text-align:center"><div style="font-size:9px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;color:#1a7a40;margin-bottom:6px">Revenue Uplift from ONDC</div><div style="font-size:18px;font-weight:900;color:#1a7a40;font-family:monospace">+' + f"{uplift_pct:.1f}" + '%</div><div style="font-size:10px;color:#7A92AA;margin-top:3px">' + fmt_inr(ondc_pos) + ' incremental via ONDC marketplace</div></div>' if ondc_data_available else '<div style="background:#FAFAFA;border:1px solid #E0E0E0;border-radius:10px;padding:14px 12px;border-top:3px solid #BDBDBD;text-align:center"><div style="font-size:9px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;color:#9E9E9E;margin-bottom:6px">Revenue Uplift from ONDC</div><div style="font-size:14px;font-weight:700;color:#9E9E9E;font-family:monospace">N/A</div><div style="font-size:10px;color:#BDBDBD;margin-top:3px">ONDC not yet activated</div></div>'}
    <div style="background:#FFFFFF;border:1px solid #{'F5C6CB' if avg_ret_rate>=7 else ('FFE8A1' if avg_ret_rate>=4 else 'C3E6CB')};border-radius:10px;padding:14px 12px;border-top:3px solid {'#e74c3c' if avg_ret_rate>=7 else ('#f39c12' if avg_ret_rate>=4 else '#27ae60')};text-align:center">
      <div style="font-size:9px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;color:#7A92AA;margin-bottom:6px">Product Return Rate</div>
    ...
    <div style="background:#FFFFFF;border:1px solid #E3D0F5;border-radius:10px;padding:14px 12px;border-top:3px solid #8b5cf6;text-align:center">
      <div style="font-size:9px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;color:#7A92AA;margin-bottom:6px">Total Replacement Units</div>
      <div style="font-size:18px;font-weight:900;color:#6D28D9;font-family:monospace">{replacements:,}</div>
      <div style="font-size:10px;color:#7A92AA;margin-top:3px">total replacement units issued</div>
    </div>
  </div>

  <!-- SECTION 2: Revenue Breakdown -->
  <div style="font-size:11px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;color:#1B4F8A;margin-bottom:10px;margin-top:4px;padding-bottom:6px;border-bottom:2px solid #D8E8F8">💰 Revenue Breakdown — Retail vs ONDC Marketplace</div>
  {"" if not ondc_data_available else ""}
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:16px">
    <div style="background:#F8FAFF;border:1px solid #D0E4F4;border-radius:10px;padding:14px 16px">
      <div style="font-size:9px;font-weight:800;letter-spacing:1px;text-transform:uppercase;color:#7A92AA;margin-bottom:5px">📊 Baseline Retail Revenue (Pre-ONDC)</div>
      <div style="font-size:20px;font-weight:900;color:#7A92AA;font-family:monospace">{fmt_inr(rev_before) if ondc_data_available else fmt_inr(total_gross)}</div>
      <div style="font-size:11px;color:#7A92AA;margin-top:4px">{"Your baseline retail revenue before ONDC adoption" if ondc_data_available else "Total retail revenue across all channels"}</div>
    </div>
    <div style="background:{"#EAF7EE" if ondc_data_available else "#F5F5F5"};border:1px solid {"#C3E6CB" if ondc_data_available else "#E0E0E0"};border-radius:10px;padding:14px 16px">
      <div style="font-size:9px;font-weight:800;letter-spacing:1px;text-transform:uppercase;color:{"#1a7a40" if ondc_data_available else "#9E9E9E"};margin-bottom:5px">🚀 ONDC Marketplace Revenue</div>
      <div style="font-size:20px;font-weight:900;color:{"#1a7a40" if ondc_data_available else "#9E9E9E"};font-family:monospace">{fmt_inr(ondc_pos) if ondc_data_available else "—"}</div>
      <div style="font-size:11px;color:#7A92AA;margin-top:4px">{"Incremental revenue generated through ONDC marketplaces" if ondc_data_available else "No ONDC marketplace data detected in dataset"}</div>
    </div>
    <div style="background:#F0F7FF;border:1px solid #B8D4F0;border-radius:10px;padding:14px 16px">
      <div style="font-size:9px;font-weight:800;letter-spacing:1px;text-transform:uppercase;color:#1B4F8A;margin-bottom:5px">📈 Total Revenue After ONDC Adoption</div>
      <div style="font-size:20px;font-weight:900;color:#1B4F8A;font-family:monospace">{fmt_inr(total_net)}</div>
      <div style="font-size:11px;color:#7A92AA;margin-top:4px">{"" + str(_upl_bdg(uplift_pct)) + " uplift vs baseline retail revenue" if ondc_data_available else "Total retail revenue (ONDC not yet active)"}</div>
    </div>
  </div>
  ...
  <div style="margin-bottom:16px">
    <div style="font-size:11px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;
      ...
      ⚙️ Operational Metrics — Fulfilment &amp; Quality Indicators
    </div>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px">
      ...
      <div style="background:#FFF5F5;border:1px solid #F5C6CB;border-radius:10px;padding:14px 14px;border-top:3px solid #e74c3c">
        <div style="font-size:9px;font-weight:800;letter-spacing:1.2px;text-transform:uppercase;color:#7A92AA;margin-bottom:6px">Total Units Returned</div>
        <div style="font-size:20px;font-weight:900;color:#e74c3c;font-family:monospace">{qty_returned:,}</div>
        <div style="font-size:10px;color:#7A92AA;margin-top:4px">units returned · Benchmark &lt;7%</div>
      </div>
      <!-- Replacements -->
      <div style="background:#F9F5FF;border:1px solid #E3D0F5;border-radius:10px;padding:14px 14px;border-top:3px solid #8b5cf6">
        <div style="font-size:9px;font-weight:800;letter-spacing:1.2px;text-transform:uppercase;color:#7A92AA;margin-bottom:6px">Total Replacement Units</div>
        <div style="font-size:20px;font-weight:900;color:#6D28D9;font-family:monospace">{replacements:,}</div>
        <div style="font-size:10px;color:#7A92AA;margin-top:4px">units replaced · QC &amp; quality impact</div>
      </div>
      ...
      <div style="background:#EAF7EE;border:1px solid #C3E6CB;border-radius:10px;padding:14px 14px;border-top:3px solid #27ae60">
        <div style="font-size:9px;font-weight:800;letter-spacing:1.2px;text-transform:uppercase;color:#7A92AA;margin-bottom:6px">Order Fulfilment Rate</div>
        <div style="font-size:20px;font-weight:900;color:#1a7a40;font-family:monospace">{(100 - avg_ret_rate):.1f}%</div>
        <div style="font-size:10px;color:#7A92AA;margin-top:4px">orders successfully delivered · Target &gt;93%</div>
      </div>
    </div>
  </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      <div style="display:flex;gap:8px;align-items:flex-start">
        ...
        <span style="font-size:12px;color:#D0EAFF;line-height:1.6">
          ONDC marketplace adoption generated <strong style="color:#52e88a">{fmt_inr(ondc_pos)}</strong> in incremental marketplace revenue
          {("— a " + f"{uplift_pct:.1f}% uplift over baseline retail revenue." if uplift_pct > 0 else ". Activate ONDC to unlock incremental marketplace revenue.")}
        </span>
      </div>
      <div style="display:flex;gap:8px;align-items:flex-start">
        ...
        <span style="font-size:12px;color:#D0EAFF;line-height:1.6">
          ...
          {'is above the 7% benchmark — an immediate SKU-level quality audit is recommended.' if avg_ret_rate>=7 else 'is within the healthy benchmark (&lt;7%) — product quality is performing well.'}
        </span>
      </div>
      <div style="display:flex;gap:8px;align-items:flex-start">
        <span style="color:#A8D8FF;font-size:14px;line-height:1.5">•</span>
        <span style="font-size:12px;color:#D0EAFF;line-height:1.6">
          Store-level operational performance is <strong style="color:#A8D8FF">tracked across all locations</strong>.
          Order fulfilment rate: {(100 - avg_ret_rate):.1f}% — indicating strong delivery performance.
        </span>
      </div>
      <div style="display:flex;gap:8px;align-items:flex-start">
        <span style="color:#A8D8FF;font-size:14px;line-height:1.5">•</span>
        <span style="font-size:12px;color:#D0EAFF;line-height:1.6">
          Marketplace expansion through <strong style="color:#A8D8FF">ONDC-enabled platforms</strong> is recommended
          based on your {biz_type} business profile, margin health, and return quality.
        </span>
      </div>
    </div>
  </div>
        ...
        _ai_ret_li = (f'<li>🔴 <strong>Return rate {fmt_pct(avg_ret_rate)} exceeds the 7% operational benchmark.</strong> '
                      f'Audit top returned SKUs to identify packaging, product quality, or logistics issues. '
                      ...
                      f'<li>✅ <strong>Product return rate remains healthy at {fmt_pct(avg_ret_rate)}</strong>, well within the operational benchmark (&lt;7%), '
                      f'indicating stable product quality and fulfilment performance. Maintain current quality control processes.</li>')

        _ai_upl_li = (f'<li>🔴 <strong>ONDC revenue uplift is currently {fmt_pct(uplift_pct)}, below the 10% adoption threshold.</strong> '
                      f'Review SNP listing quality, pricing competitiveness, and catalogue completeness to accelerate marketplace growth.</li>'
                      if uplift_pct < 10 else
                      f'<li>✅ <strong>ONDC marketplace integration generated a {fmt_pct(uplift_pct)} revenue uplift over the baseline retail business.</strong> '
                      f'Expand the product catalogue on high-performing ONDC seller platforms to sustain this growth trajectory.</li>')
        _margin_gap6 = max(0, 20 - avg_margin)
        _ai_mar_li  = (f'<li>🟡 <strong>Average profit margin of {fmt_pct(avg_margin)} is below the 20% target.</strong> '
                       f'Renegotiate supplier costs or revise pricing strategy to close the {_margin_gap6:.1f}pp gap and improve marketplace competitiveness.</li>'
                       if avg_margin < 20 else
                       f'<li>✅ <strong>Average profit margin of {fmt_pct(avg_margin)} provides strong headroom for competitive ONDC pricing strategies.</strong> '
                       f'Consider scaling additional SKUs on ONDC marketplaces to accelerate revenue growth.</li>')

        # ONDC channel revenue — reworded for clarity
        _ai_ondc_li = (f'<li>📦 <strong>ONDC marketplace channel contributed {fmt_inr(ondc_pos)} in incremental revenue, '
                       f'representing a {fmt_pct(uplift_pct)} uplift over the pre-ONDC retail baseline.</strong> '
                       f'Further expansion can be achieved by onboarding remaining SKUs onto suitable ONDC Seller Network Participants.</li>'
                       if ondc_pos > 0 else
                       f'<li>🚀 <strong>No ONDC channel revenue detected yet.</strong> '
                       f'Initiating SNP onboarding could generate a 10% uplift on the current gross sales of {fmt_inr(total_gross)}, adding significant incremental revenue.</li>')

        _ai_rep_li  = (f'<li>🔁 <strong>{replacements} replacement units were issued during this period,</strong> '
                       f'suggesting moderate after-sales service activity. This may indicate opportunities to strengthen '
                       f'pre-dispatch quality checks or logistics handling to reduce replacement-related costs.</li>'
                       if replacements > 50 else
                       f'<li>✅ <strong>Replacement units remain low at {replacements} units</strong>, indicating strong product quality and fulfilment standards. '
                       f'Continue preventive quality control practices to sustain this performance.</li>')
        _ai_hlt_li  = (f'<li>💡 <strong>An ONDC Readiness Score of {fmt_pct(health_score)} indicates the business is well-positioned for accelerated marketplace integration.</strong> '
                       f'Complete SNP onboarding and catalogue expansion to fully unlock the digital marketplace opportunity.</li>'
                       if health_score >= 65 else
                       f'<li>⚠️ <strong>ONDC Readiness Score of {fmt_pct(health_score)} indicates readiness improvement is required before full marketplace activation.</strong> '
                       f'Priority actions: reduce financial risk score below 0.40 and raise average margin above 20%.</li>')
        _ai_net_li  = (f'<li>📊 <strong>Net sales post-ONDC integration stand at {fmt_inr(total_net)}</strong>, compared to a pre-ONDC baseline of {fmt_inr(rev_before)}. ' +
                       ('The positive revenue trajectory confirms strong marketplace adoption momentum. Continue scaling ONDC channel presence.</li>'
                        if total_net >= rev_before else
                        'A revenue decline post-ONDC integration warrants review of pricing strategy, SNP listing quality, and platform visibility.</li>'))
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
      <div style="background:#fff;border-radius:10px;padding:14px 16px;border-left:4px solid #e74c3c;">
        ...
        <ul style="margin:0;padding-left:0;list-style:none;font-size:12px;line-height:1.9;color:#1A2D45">
          {_ai_ret_li}{_ai_upl_li}{_ai_mar_li}
        </ul>
      </div>
...
"""
        ...
        if has_ts and gc and pmrc:
            ...
            ax1b = ax1.twinx()
            x1   = range(len(q1))
            bars1 = ax1.bar(x1, q1['sales']/1e5, color=NAVY, alpha=0.72, label='Total Business Revenue (₹L)', width=0.6, zorder=3)
            ax1b.plot(x1, q1['margin'], color=RED, linewidth=2.5, marker='o', markersize=5, label='Profit Margin (%)', zorder=4)
            ax1.set_xticks(list(x1))
            ax1.set_xticklabels(q1['_ql'], rotation=45, ha='right', fontsize=11)
            ax1.set_ylabel('Revenue (₹ Lakhs)', fontsize=12, fontweight='bold', color=NAVY)
            ax1b.set_ylabel('Profit Margin (%)', fontsize=12, fontweight='bold', color=RED)
            fig1.text(0.01, 0.95, 'Revenue vs Profit Margin (Quarterly)', fontsize=14, fontweight='bold', color='#0B1F3A', va='top', transform=fig1.transFigure)
            ...
            fig1.text(0.01, 0.95, 'Revenue vs Profit Margin', fontsize=14, fontweight='bold', color='#0B1F3A', va='top', transform=fig1.transFigure)
        ...
        if has_ts and boc and ochc and ondc_data_available:
            ...
            x2 = range(len(q2))
            ax2.bar(x2, q2['before']/1e5,  label='Baseline Retail Revenue', color='#7A92AA', alpha=0.80, width=0.6, zorder=3)
            ax2.bar(x2, q2['ondc_p']/1e5, bottom=q2['before']/1e5,
                    label='ONDC Marketplace Revenue', color=GREEN, alpha=0.85, width=0.6, zorder=3)
            ax2.plot(x2, q2['gross']/1e5, color=NAVY, linewidth=2.5, marker='D', markersize=5,
                     label='Total Business Revenue', zorder=5)
            ...
            if first_live:
                ...
                ax2.text(li - 0.4, ax2.get_ylim()[1] * 0.92 if ax2.get_ylim()[1] else 1000,
                         '▶ ONDC Live', fontsize=12, color=GREEN, fontweight='bold')
            ax2.set_xticks(list(x2))
            ax2.set_xticklabels(q2['_ql'], rotation=45, ha='right', fontsize=11)
            ax2.set_ylabel('Revenue (₹ Lakhs)', fontsize=12, fontweight='bold')
            fig2.text(0.01, 0.95, 'Revenue Impact: Pre-ONDC vs Post-ONDC (Quarterly)', fontsize=14, fontweight='bold', color='#0B1F3A', va='top', transform=fig2.transFigure)
            ax2.legend(fontsize=11, loc='upper left', framealpha=0.9)
            ax2.yaxis.set_major_formatter(mticker.FuncFormatter(_inr_fmt))
        else:
            _msg2 = ('No ONDC revenue data available.\nUpload dataset with ONDC channel/revenue columns\nto activate this chart.' if not ondc_data_available else 'ONDC revenue columns not found in data')
            ax2.text(0.5, 0.55, _msg2, ha='center', va='center',
                     transform=ax2.transAxes, fontsize=11, color='#7A92AA',
                     style='italic', linespacing=1.7)
            ax2.text(0.5, 0.25, '📊 Chart activates after ONDC onboarding', ha='center', va='center',
                     transform=ax2.transAxes, fontsize=9, color='#BDBDBD')
            ax2.set_facecolor('#FAFAFA')
            for sp in ax2.spines.values(): sp.set_color('#E8E8E8')
            fig2.text(0.01, 0.95, 'Revenue Impact: Pre-ONDC vs Post-ONDC', fontsize=14, fontweight='bold', color='#0B1F3A', va='top', transform=fig2.transFigure)
        ...
        if has_ts and rrc:
            ...
            if qrc and rpc:
                b3a = ax3.bar([xi - bar_w/2 for xi in x3], q3[qrc],
                              width=bar_w, color=RED, alpha=0.75, label='Total Units Returned', zorder=3)
                b3b = ax3.bar([xi + bar_w/2 for xi in x3], q3[rpc],
                              width=bar_w, color=AMBER, alpha=0.75, label='Replacement Units', zorder=3)
            elif qrc:
                ax3.bar(x3, q3[qrc], width=0.6, color=RED, alpha=0.75, label='Total Units Returned', zorder=3)
            ax3b.plot(x3, q3[rrc],  color=PURP, linewidth=2.5, marker='o', markersize=5,
                      label='Product Return Rate %', zorder=4)
            if rlrc:
                ax3b.plot(x3, q3[rlrc], color=TEAL, linewidth=1.8, linestyle='--', marker='s',
                           markersize=4, label='6-Month Rolling Return Rate', zorder=4)
            ax3b.axhline(7, color=RED, linestyle=':', linewidth=1.2, alpha=0.5)
            ax3b.text(len(q3)-0.5, 7.2, 'Target <7%', fontsize=11, color=RED, ha='right')
            ax3.set_xticks(list(x3))
            ax3.set_xticklabels(q3['_ql'], rotation=45, ha='right', fontsize=11)
            ax3.set_ylabel('Units / Return Rate (%)', fontsize=12, fontweight='bold', color=RED)
            ax3b.set_ylabel('Product Return Rate %', fontsize=12, fontweight='bold', color=PURP)
            fig3.text(0.01, 0.95, 'Returns & Replacement Trend (Quarterly)', fontsize=14, fontweight='bold', color='#0B1F3A', va='top', transform=fig3.transFigure)
            h3a, l3a = ax3.get_legend_handles_labels();  h3b, l3b = ax3b.get_legend_handles_labels()
            ax3.legend(h3a+h3b, l3a+l3b, loc='upper left', fontsize=11, framealpha=0.9)
        else:
            _msg3 = ('No ONDC returns/replacements data available.\nAdd returns or replacement columns to activate this chart.' if not ondc_data_available else 'No returns data available')
            ax3.text(0.5, 0.55, _msg3, ha='center', va='center',
                     transform=ax3.transAxes, fontsize=11, color='#7A92AA',
                     style='italic', linespacing=1.7)
            ax3.text(0.5, 0.25, '📦 Returns tracking activates with ONDC operational data', ha='center', va='center',
                     transform=ax3.transAxes, fontsize=9, color='#BDBDBD')
            ax3.set_facecolor('#FAFAFA')
            for sp in ax3.spines.values(): sp.set_color('#E8E8E8')
            fig3.text(0.01, 0.95, 'Returns & Replacement Trend', fontsize=14, fontweight='bold', color='#0B1F3A', va='top', transform=fig3.transFigure)
        ...
        # Detect store identifier column — support common name variants
        _sid4 = next((c for c in df.columns if c.lower() in
                      ('store_id', 'store', 'store_name', 'branch', 'location',
                       'outlet', 'shop_id', 'shop', 'outlet_id')), None)
        if _sid4 and gc:
            agg4 = dict(net=(nc or gc, 'sum'), before=(boc or gc, 'sum'))
            ...
            store_lbls = []
            for _sv in st4[_sid4]:
                try:    store_lbls.append(f'Store {int(_sv)}')
                except: store_lbls.append(str(_sv))
            x4  = range(len(st4))
            w4  = 0.28
            # Left sub-plot: revenue bars
            ax4a.bar([xi - w4 for xi in x4], st4['before']/1e5,   width=w4, color='#7A92AA', alpha=0.85, label='Baseline Retail Revenue')
            ax4a.bar([xi       for xi in x4], st4['net']/1e5,     width=w4, color=NAVY,     alpha=0.85, label='Total Revenue')
            if 'ondc_p' in st4:
                ax4a.bar([xi + w4 for xi in x4], st4['ondc_p']/1e5, width=w4, color=GREEN,   alpha=0.85, label='ONDC Marketplace Revenue')
            ax4a.set_xticks(list(x4)); ax4a.set_xticklabels(store_lbls, fontsize=11)
            ax4a.set_ylabel('Revenue (₹ Lakhs)', fontsize=12, fontweight='bold')
            ax4a.set_title('Store Revenue: Baseline vs ONDC Marketplace', fontsize=14, fontweight='bold', pad=10)
            ax4a.legend(fontsize=11); ax4a.yaxis.set_major_formatter(mticker.FuncFormatter(_inr_fmt))
            ...
            ax4b.set_xticks(list(x4)); ax4b.set_xticklabels(store_lbls, fontsize=11)
            ax4b.tick_params(axis='both', labelsize=11)
            ax4b2.tick_params(axis='y', labelsize=11)
            ...
            # Distinguish: no store column vs no ONDC data
            if _sid4 is None:
                _msg4_main = 'Store-level ONDC analysis requires\na store identifier column.'
                _msg4_sub  = '🏪 Add Store_ID, Store, Branch, or Location column to enable this chart.'
            elif not ondc_data_available:
                _msg4_main = 'Store-level ONDC impact will appear\nafter ONDC sales data is available.'
                _msg4_sub  = '🏪 Activates when ONDC channel/flag columns are present in the dataset.'
            else:
                _msg4_main = 'No store-level data available for the selected filters.'
                _msg4_sub  = '🏪 Try selecting a different store or category filter.'
            for ax_ in [ax4a, ax4b]:
                ax_.text(0.5, 0.58, _msg4_main, ha='center', va='center',
                         transform=ax_.transAxes, fontsize=11, color='#7A92AA',
                         style='italic', linespacing=1.7)
                ax_.text(0.5, 0.28, _msg4_sub, ha='center', va='center',
                         transform=ax_.transAxes, fontsize=9, color='#BDBDBD',
                         linespacing=1.5)
                ax_.set_facecolor('#FAFAFA')
                for sp in ax_.spines.values(): sp.set_color('#E8E8E8')
        fig4.text(0.01, 0.96, 'Store-Level ONDC Performance Analysis', fontsize=14, fontweight='bold', color='#0B1F3A', va='top', transform=fig4.transFigure)

        cat_options = ['All Categories']
        if 'Product_Category' in df.columns:
            cat_options += sorted(df['Product_Category'].dropna().unique().tolist())
        # s1-s4 intentionally None — chart insight notes were duplicated from Step 6
        # and have been removed per UX review. sum1-sum4 components remain hidden.
...
# ── Mock Udyam / MSME registry (demo data — replace with live API in production) ───
# enterprise_name here is shown in Steps 5/6/7 headers when the uploaded dataset
# contains an MSME_ID column matching these Udyam numbers.
# If the dataset's MSME_ID is NOT in this table, _extract_business_name() falls back
# to displaying the raw MSME_ID string itself, keeping the UI data-driven.
udyam_master_data = pd.DataFrame({
    'udyam_number':    ['UDYAM-UP-01-0000001','UDYAM-TN-00-7629703','UDYAM-KL-03-0000003'],
    'enterprise_name': ['Tech Innovations Pvt Ltd','Trichy Multi-Store Retail (UDYAM-TN-00-7629703)','FMCG Distributors Kerala'],
...
def _fetch_msme_data(msme_number):
    ...
    if not fetched.empty:
        ...
        return (row['enterprise_name'], row['organisation_type'], row['major_activity'],
                ...
                "✅ MSME Data Fetched Successfully (Pilot: demo registry)")
    return "", "", "", "", "", "", "Retail", "⚠️ MSME not in pilot registry. Please fill details manually below — live Udyam API integration planned for Stage 2."

# ╔══════════════════════════════════════════════════════════════════════╗
# ║  PILLARS 2 & 3  — AI Categorisation + UDYAM Verify + Catalogue      ║
# ╚══════════════════════════════════════════════════════════════════════╝

# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  PILLAR 2  —  AI Product Categorisation (ONDC Taxonomy)                    ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

import re as _re_p2

ONDC_TAXONOMY = {
    "Grocery & Staples":     ["rice","wheat","dal","flour","oil","sugar","salt","atta","pulses","grain","maida","suji","poha","murmura"],
    "Fruits & Vegetables":   ["vegetable","fruit","tomato","onion","potato","carrot","leafy","fresh produce","brinjal","capsicum","cabbage","spinach","banana","mango","apple"],
    "Dairy & Eggs":          ["milk","curd","paneer","cheese","butter","ghee","egg","yogurt","dairy","lassi","cream","khoya"],
    "Meat & Seafood":        ["chicken","mutton","fish","prawn","meat","seafood","poultry","lamb","beef","pork","crab","lobster"],
    "Beverages":             ["juice","water","tea","coffee","soft drink","energy drink","cola","lassi","sharbat","squash","syrup","nimbu pani"],
    "Snacks & Branded Food": ["biscuit","chips","namkeen","chocolate","candy","noodle","instant","wafer","popcorn","kurkure","mixture","mathri","papad"],
    "Personal Care":         ["shampoo","soap","toothpaste","lotion","cream","face wash","deodorant","perfume","moisturiser","conditioner","body wash","talcum"],
    "Home Care":             ["detergent","cleaner","dishwash","mop","broom","phenyl","sanitizer","floor cleaner","toilet cleaner","scrubber","cloth","duster"],
    "Electronics":           ["mobile","phone","laptop","charger","cable","earphone","tablet","battery","led","bulb","fan","iron","mixer","grinder","tv","television","remote"],
    "Apparel & Clothing":    ["shirt","pant","saree","kurta","dress","jacket","jean","trouser","fabric","cloth","dupatta","lehenga","salwar","kurti","top","blouse"],
    "Footwear":              ["shoe","sandal","slipper","boot","chappal","footwear","heel","sneaker","loafer","moccasin"],
    "Home & Kitchen":        ["utensil","pressure cooker","vessel","plate","bowl","glass","tiffin","dabba","spoon","ladle","kadai","pan","tawa"],
    "Furniture":             ["chair","table","sofa","bed","wardrobe","shelf","rack","cabinet","almirah","stool","bench","desk","mattress"],
    "Stationery & Books":    ["pen","pencil","notebook","paper","book","register","file","folder","eraser","scale","geometry","crayon","marker"],
    "Health & Pharma":       ["medicine","tablet","capsule","syrup","supplement","vitamin","bandage","thermometer","sanitizer","mask","gloves","ointment"],
    "Agricultural Inputs":   ["fertilizer","seed","pesticide","tractor","pump","farm","agricultural","irrigation","sprayer","sickle","plough"],
    "Industrial Supplies":   ["bolt","nut","pipe","valve","wire","cable","tool","machine part","bearing","gear","motor","pump","drill","grease"],
    "Services":              ["repair","service","maintenance","installation","consulting","training","coaching","delivery","transport","laundry","cleaning"],
}

def classify_product_tfidf(description: str) -> tuple:
    """Keyword-overlap fallback classifier. Returns (category, confidence 0-1)."""
    desc_lower = str(description).lower()
    best_cat, best_score = "General Merchandise", 0
    for cat, keywords in ONDC_TAXONOMY.items():
...
def classify_products_batch(df, sku_col: str, cat_col: str = None, use_llm: bool = True):
    """
    Classify every unique SKU in df into ONDC taxonomy.
    Adds two columns: ONDC_Category and ONDC_Confidence.
    Uses Claude API first (if available), falls back to TF-IDF keyword match.
    """
    import json as _json_p2, requests as _req_p2
    df = df.copy()
    df["ONDC_Category"]   = "General Merchandise"
    df["ONDC_Confidence"] = 0.0

    if sku_col not in df.columns:
        return df

    unique_skus = [str(s) for s in df[sku_col].dropna().unique().tolist() if str(s).strip()]
    if not unique_skus:
        return df

    results = {}

    # ── Path A: Claude API batch (up to 50 SKUs per call) ─────────────────
    if use_llm:
        try:
            batch_size = 50
            for i in range(0, len(unique_skus), batch_size):
                ...
                prompt = (
                    "You are an ONDC product taxonomy classifier for Indian MSMEs.\n"
                    "Classify each product into exactly one of these ONDC L1 categories:\n"
                    + ", ".join(ONDC_TAXONOMY.keys()) + "\n\n"
                    "Return ONLY a JSON object mapping each product name to its category. "
                    "No explanation, no markdown.\n"
                    "Example: {\"Rice 5kg\": \"Grocery & Staples\", \"Samsung Charger\": \"Electronics\"}\n\n"
                    "Products:\n" + _json_p2.dumps(batch)
                )
                resp = _req_p2.post(
                ...
                if resp.status_code == 200:
                    ...
                    text = _re_p2.sub(r"```json|```", "", text).strip()
                    parsed = _json_p2.loads(text)
                    for sku, cat in parsed.items():
                        if cat in ONDC_TAXONOMY:
                            results[sku] = (cat, 0.93)
        except Exception:
            pass

    # ── Path B: TF-IDF fallback for unclassified SKUs ─────────────────────
    for sku in unique_skus:
        if sku not in results:
            cat_hint = ""
            if cat_col and cat_col in df.columns:
                rows = df[df[sku_col] == sku][cat_col]
                if not rows.empty:
                    cat_hint = str(rows.iloc[0])
            cat, conf = classify_product_tfidf(str(sku), category_hint=cat_hint)
            results[sku] = (cat, conf)

    df["ONDC_Category"]   = df[sku_col].map(lambda s: results.get(str(s), ("General Merchandise", 0.0))[0])
    df["ONDC_Confidence"] = df[sku_col].map(lambda s: results.get(str(s), ("General Merchandise", 0.0))[1])
    return df


# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  PILLAR 3A  —  UDYAM Format Verification                                   ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

_VALID_UDYAM_STATES_P3 = {
...
def verify_udyam_number(udyam: str) -> dict:
    """
    Validate a UDYAM registration number by format and state-code check.
    Returns dict with keys: valid, formatted, state, error, confidence.
    """
    if not udyam or not isinstance(udyam, str):
        return {"valid": False, "formatted": "", "state": "", "error": "No number provided", "confidence": "Format Invalid"}
    cleaned = _re_p2.sub(r"[\s]+", "-", udyam.strip().upper())
    cleaned = _re_p2.sub(r"-{2,}", "-", cleaned)
    if not cleaned.startswith("UDYAM"):
        cleaned = "UDYAM-" + cleaned
    m = _re_p2.match(r'^UDYAM-([A-Z]{2})-(\d{2})-(\d{7})$', cleaned)
    if not m:
        return {"valid": False, "formatted": cleaned, "state": "", "error": "Format must be UDYAM-XX-00-0000000", "confidence": "Format Invalid"}
    state_code = m.group(1)
    district   = int(m.group(2))
    if state_code not in _VALID_UDYAM_STATES_P3:
        return {"valid": False, "formatted": cleaned, "state": "", "error": f"Unknown state code '{state_code}'", "confidence": "Format Invalid"}
    if district > 99:
        return {"valid": False, "formatted": cleaned, "state": _UDYAM_STATE_P3.get(state_code,""), "error": "District code out of range (00–99)", "confidence": "Format Invalid"}
    return {"valid": True, "formatted": cleaned, "state": _UDYAM_STATE_P3.get(state_code,""), "error": "", "confidence": "Format Valid"}

def render_udyam_verify_html(result: dict) -> str:
    """Return a compact inline badge for the Step 2 UI."""
    if not result:
        return ""
    if result["valid"]:
        return (
            "<div style='display:flex;align-items:center;gap:8px;padding:7px 12px;"
            "background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;margin-top:4px;'>"
            "<span style='color:#16a34a;font-size:14px;'>✔</span>"
            f"<span style='font-size:12px;color:#166534;'><strong>UDYAM format valid</strong>"
            f" · State: {result['state']} · {result['formatted']}</span></div>"
        )
    return (
        "<div style='display:flex;align-items:center;gap:8px;padding:7px 12px;"
        "background:#fef2f2;border:1px solid #fecaca;border-radius:6px;margin-top:4px;'>"
        "<span style='color:#dc2626;font-size:14px;'>✖</span>"
        f"<span style='font-size:12px;color:#991b1b;'><strong>Invalid UDYAM:</strong> {result['error']}</span></div>"
    )


# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  PILLAR 3B  —  ONDC Catalogue Generator                                    ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

def generate_ondc_catalogue(df, user_data: dict, use_llm: bool = True) -> tuple:
    """
    Build an ONDC-schema product catalogue from scored sales data.
    Returns (catalogue_list, html_preview_string).
    """
    import json as _json_p3, requests as _req_p3, tempfile as _tmp_p3, csv as _csv_p3
    sku_col   = next((c for c in df.columns if c in ("SKU_Name","Product_Name","product_name","Item_Name")), None)
    # Prefer ONDC_Category (added by Pillar 2) over raw Category column
    ondc_cat_col = "ONDC_Category" if "ONDC_Category" in df.columns else None
    raw_cat_col  = next((c for c in df.columns if c in ("Product_Category","Category","category")), None)
    cat_col      = ondc_cat_col or raw_cat_col
    sal_col = "Monthly_Sales_INR"
    mar_col = "Avg_Margin_Percent"
    ret_col = "Returns_Percentage"
    inv_col = "Inventory_Turnover"
    # Unit_Price gives the actual selling price per unit
    price_col = next((c for c in df.columns if c in ("Unit_Price","unit_price","Price","price","MRP","mrp")), None)

    if not sku_col:
        return [], "<p style='color:#e74c3c;font-size:12px;'>No product name column found in dataset.</p>"

    agg = {}
    for c in [sal_col, mar_col, ret_col, inv_col]:
        if c in df.columns:
            agg[c] = "mean" if c != sal_col else "sum"
    if price_col and price_col in df.columns:
        agg[price_col] = "mean"
    if cat_col and cat_col in df.columns:
        agg[cat_col] = "first"

    sku_df = df.groupby(sku_col).agg(agg).reset_index() if agg else df[[sku_col]].drop_duplicates()

    # ── Optional: AI-generated product descriptions ────────────────────────
    descriptions = {}
    if use_llm and len(sku_df) > 0:
        try:
            skus_list = sku_df[sku_col].tolist()[:30]
            prompt = (
                "You are an ONDC catalogue assistant for Indian MSMEs.\n"
                "For each product name, write a concise 1-sentence description (max 100 characters) "
                "suitable for an ONDC marketplace listing. "
                "Return ONLY a JSON object: {\"product_name\": \"description\", ...}\n\n"
                "Products:\n" + _json_p3.dumps(skus_list)
            )
            resp = _req_p3.post(
                "https://api.anthropic.com/v1/messages",
                headers={
            ...
            if resp.status_code == 200:
                text = resp.json()["content"][0]["text"]
                text = _re_p2.sub(r"```json|```", "", text).strip()
                descriptions = _json_p3.loads(text)
        except Exception:
            pass

    seller_id = user_data.get("msme_number", "UDYAM-XX-00-0000000")
    company   = user_data.get("company_name", "MSME Business")
    catalogue = []

    for _, row in sku_df.iterrows():
        sku_name  = str(row[sku_col])
        ondc_cat  = str(row.get(cat_col, "General Merchandise")) if cat_col and cat_col in row else "General Merchandise"
        avg_sales = float(row.get(sal_col, 0))
        margin    = float(row.get(mar_col, 15))
        ret_rate  = float(row.get(ret_col, 5))
        turnover  = float(row.get(inv_col, 3))
        # Use actual unit price if available, else estimate from revenue
        if price_col and price_col in row.index and float(row.get(price_col, 0)) > 0:
            est_price = round(float(row[price_col]), 2)
        else:
            # Fallback: avg monthly revenue / avg units sold
            est_price = round(avg_sales / max(turnover * 30, 1), 2)
        ondc_ready = ret_rate < 10 and margin > 5
        catalogue.append({
            "seller_id":           seller_id,
            "seller_name":         company,
            "product_id":          _re_p2.sub(r"[^A-Z0-9]", "", sku_name.upper())[:20] or "SKU001",
            "product_name":        sku_name,
            "ondc_category_l1":    ondc_cat,
            "description":         descriptions.get(sku_name, f"{sku_name} — quality product by {company}"),
            "estimated_price_inr": est_price,
            "avg_margin_pct":      round(margin, 1),
            "return_rate_pct":     round(ret_rate, 1),
            "ondc_listing_ready":  ondc_ready,
            "fulfilment_mode":     "seller-fulfilled",
            "hsn_code":            "",
            "gst_rate_pct":        "",
        })
    ...
    rows_html = ""
    for e in catalogue:
        _m  = float(e.get('avg_margin_pct', 0))
        _r  = float(e.get('return_rate_pct', 0))
        _rdy = e['ondc_listing_ready']
        # Readiness reason — relative thresholds so reasons are
        # distributed meaningfully across the actual dataset.
        # Median margin ~33%, median return ~0.90% for this dataset.
        _m_high = 33.0   # top third of dataset margin
        _m_mid  = 20.0   # mid-range margin threshold
        _r_low  = 0.90   # below-median return rate
        if _rdy and _m >= _m_high and _r < _r_low:
            _reason = "Strong margin and below-average return rate"
        elif _rdy and _m >= _m_high:
            _reason = "Strong profit margin with stable returns"
        elif _rdy and _r < _r_low:
            _reason = "Low return rate with consistent demand"
        elif _rdy and _m >= _m_mid:
            _reason = "Good margin with acceptable return rate"
        elif _rdy:
            _reason = "Meets ONDC listing requirements"
        elif not _rdy and _r >= 10:
            _reason = "Return rate above threshold — needs review"
        elif not _rdy and _m <= 5:
            _reason = "Low margin may affect marketplace viability"
        else:
            _reason = "Margin or return metrics need improvement"
        badge = ("<span style='color:#16a34a;font-weight:700;font-size:11px;'>✔ Ready for ONDC Listing</span>"
                 if _rdy
                 else "<span style='color:#d97706;font-weight:700;font-size:11px;'>⚠ Review Required</span>")
        rows_html += (
            f"<tr style='border-bottom:1px solid #EEF4FB;'>"
            f"<td style='padding:8px 12px;font-size:12px;color:#0B1F3A;'>{e['product_name']}</td>"
            f"<td style='padding:8px 12px;font-size:12px;color:#0B1F3A;'>{e['ondc_category_l1']}</td>"
            f"<td style='padding:8px 12px;font-size:12px;text-align:right;'>₹{e['estimated_price_inr']:,.0f}</td>"
            f"<td style='padding:8px 12px;font-size:12px;text-align:right;'>{e['avg_margin_pct']}%</td>"
            f"<td style='padding:8px 12px;text-align:center;'>{badge}</td>"
            f"<td style='padding:8px 12px;font-size:11px;color:#475569;'>{_reason}</td></tr>"
        )

    html = f"""
    <div style='margin-top:12px;border:1px solid #C8DCEF;border-radius:10px;overflow:hidden;'>
      <div style='background:#0B1F3A;padding:10px 16px;display:flex;justify-content:space-between;align-items:center;'>
        <span style='color:#A8D8FF;font-size:11px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;'>
          📦 ONDC Marketplace Product Catalogue — {len(catalogue)} SKUs
        </span>
        <span style='color:#A8D8FF;font-size:10px;'>Seller: {company}</span>
      </div>
      <table style='width:100%;border-collapse:collapse;background:#fff;'>
        <thead>
          <tr style='background:#EEF4FB;'>
            <th style='padding:8px 12px;text-align:left;font-size:11px;color:#0B1F3A;font-weight:700;'>Product</th>
            <th style='padding:8px 12px;text-align:left;font-size:11px;color:#0B1F3A;font-weight:700;'>ONDC Category</th>
            <th style='padding:8px 12px;text-align:right;font-size:11px;color:#0B1F3A;font-weight:700;'>Estimated Price</th>
            <th style='padding:8px 12px;text-align:right;font-size:11px;color:#0B1F3A;font-weight:700;'>Profit Margin %</th>
            <th style='padding:8px 12px;text-align:center;font-size:11px;color:#0B1F3A;font-weight:700;'>ONDC Listing Status</th>
            <th style='padding:8px 12px;text-align:left;font-size:11px;color:#0B1F3A;font-weight:700;'>Readiness Reason</th>
          </tr>
        </thead>
...
def export_catalogue_csv(catalogue: list) -> str:
    """Return base64-encoded JS download HTML for the catalogue CSV.
    Works in Colab/Codatalab - no server file path used."""
    import csv as _csv_p3, io as _io_p3, base64 as _b64_p3
    if not catalogue:
        return None
    buf = _io_p3.StringIO()
    writer = _csv_p3.DictWriter(buf, fieldnames=list(catalogue[0].keys()))
    writer.writeheader()
    writer.writerows(catalogue)
    b64 = _b64_p3.b64encode(buf.getvalue().encode("utf-8")).decode()
    link = (
        ...
        "<a href='data:text/csv;base64," + b64 + "' download='DataNetra_ONDC_Catalogue.csv' "
        "style='font-size:13px;font-weight:600;color:#166534;text-decoration:none;"
        "border-bottom:1px solid #166534;'>Click here to download ONDC Catalogue CSV</a>"
        "</div>"
    )
    return link
...
def generate_bi_report_pdf(user_data, df_raw, dashboard_data, granular_data):
    ...
    try:
        ...
        # =====================================================================
        company   = _extract_business_name(df_raw, user_data)
        ...
        _chart('chart1', 'Revenue vs Profit Margin (Quarterly)', w=15, h=7)
        ...
        _chart('chart4', 'Store-Level ONDC Performance Analysis', w=15, h=7)
        story.append(Spacer(1, 0.2*cm))
        story.append(_ai_insight(
      ...
      <div style="font-size:10.5px;color:rgba(255,255,255,0.75);line-height:1.5;">Forecasting, scoring<br>&amp; actionable insights</div>
        ...
        with gr.Column(elem_id="drc-block"):
            ...
            with gr.Row(equal_height=True):
                ...
                drc_tmpl_file = gr.File(label="", visible=False, show_label=False)
...
(function(){
  ...
  if(panel){ panel.scrollIntoView({behavior:'smooth',block:'start'}); }
    ...
    with gr.Column(visible=False) as step1_col:
        ...
        # Input fields rendered OUTSIDE gr.Tabs to avoid Gradio 4.20+ lazy-render bug
        # (components inside gr.Tab are not mounted until the tab is clicked on first load)
        ...
        # Voice tab kept as optional helper — fields above are always visible
...
3. Voice will auto-fill the fields above automatically""")
            # Mirror fields so they stay in sync (voice fills via JS into the elem_id targets)
            name_input_v   = gr.Textbox(label="Name (voice filled — verify above)", elem_id="step1-name-v", visible=False)
            mobile_input_v = gr.Textbox(label="Mobile (voice filled — verify above)", elem_id="step1-mobile-v", visible=False)
            role_input_v   = gr.Textbox(label="Role (voice filled)", visible=False)
            gr.Markdown("*Voice fills the fields above directly. This accordion can be closed after speaking.*")
    ...
    with gr.Column(visible=False) as step2_col:
        ...
        udyam_verify_html = gr.HTML("", elem_id="udyam-verify-badge")
        fetch_status      = gr.Markdown()

        # Voice entry as collapsible accordion — fields above always visible
        with gr.Accordion("🎙️ Voice Input (optional)", open=False):
            gr.HTML(VOICE_JS_STEP2)
            gr.Markdown("""**How it works:**
1. Click **🎤 Start Voice Input**
2. Say: *"UDYAM [Your Number], OTP [4-digit code]"*
3. Voice auto-fills the fields above — then click **Fetch MSME Data**

*🌐 Multilingual voice support (Hindi & regional languages) — coming soon*""")
            msme_voice_display = gr.Textbox(label="UDYAM Number (voice filled — verify above)", elem_id="step2-msme-v", visible=False)
            otp_voice_display  = gr.Textbox(label="OTP (voice filled — verify above)", type="password", elem_id="step2-otp-v", visible=False)

        gr.Markdown("### Fetched MSME Details")
        fetched_name     = gr.Textbox(label="Enterprise Name", interactive=True)
        fetched_org      = gr.Textbox(label="Organisation Type", interactive=True)
        fetched_activity = gr.Textbox(label="Major Activity", interactive=True)
        fetched_type     = gr.Textbox(label="Enterprise Type", interactive=True)
        fetched_state    = gr.Textbox(label="State", interactive=True)
        fetched_city     = gr.Textbox(label="City", interactive=True)
        # Industry Domain field in Step 2
        fetched_industry = gr.Textbox(label="Industry Domain", interactive=True, value="Retail")
    ...
    with gr.Column(visible=False) as step5_col:
        ...
        # ── Pillar 3B: ONDC Catalogue (shown after analysis) ─────────────────
        with gr.Column(visible=False, elem_id="ondc-catalogue-col") as ondc_cat_col:
            ondc_catalogue_html = gr.HTML("")
            ondc_catalogue_dl   = gr.HTML("", visible=False)
    ...
    with gr.Column(visible=False) as step6_col:
        gr.HTML('''<div style="background:linear-gradient(135deg,#0f2557,#1a3a6b);
          ...
          <h1 style="color:white;margin:0;font-size:1.45rem;font-weight:800;letter-spacing:0.4px">
            ONDC Business Impact &amp; Marketplace Performance
          </h1></div>''')
        kpi_table_dash = gr.HTML(value="", elem_id="kpi-table-container")
        # Performance Visualizations label removed per UX review
        gr.HTML('<div style="font-size:18px;font-weight:700;color:#0B1F3A;padding:14px 0 6px;border-top:3px solid #1B4F8A;margin-top:12px;letter-spacing:0">📊 Retail Performance &amp; Profitability Analysis</div>')
        with gr.Row():
            with gr.Column():
        ...
        gr.HTML('<div style="font-size:18px;font-weight:700;color:#0B1F3A;padding:14px 0 6px;border-top:3px solid #1B4F8A;margin-top:12px;letter-spacing:0">📦 Store-Level ONDC Marketplace Performance</div>')
        with gr.Row():
            with gr.Column():
        ...
        with gr.Row():
            forecast_deepdive_btn = gr.Button("🚀 Continue to AI Forecast & Product Intelligence →", variant="primary", size="lg")
    ...
    with gr.Column(visible=False) as step7_col:
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
            <div>
              ...
              <h2 style="color:white;margin:0;font-size:1.25rem;font-weight:800">
                Store &#8594; Category &#8594; Product Performance Intelligence &amp; Sales Forecasting
              </h2>
            </div>
            <div style="background:rgba(255,255,255,0.1);border-radius:8px;padding:8px 16px;
              ...
              Drill down by Store, Category, or Product to explore performance insights.<br>
        ...
        gr.HTML('<div style="font-size:18px;font-weight:700;letter-spacing:0;text-transform:none;'
                'color:#0B1F3A;padding:12px 0 6px;border-top:3px solid #1B4F8A;margin-top:8px">'
                '📈 Category Performance Trends (36 Months)</div>')
        ...
        gr.HTML('<div style="font-size:18px;font-weight:700;letter-spacing:0;text-transform:none;'
                'color:#0B1F3A;padding:12px 0 4px;border-top:3px solid #1B4F8A;margin-top:8px">'
                '🔮 AI Sales Forecast &amp; Demand Projection (Next 6–12 Months)'
                '<div style="font-size:12px;font-weight:400;letter-spacing:0;'
                'color:#4A6A8A;margin-top:4px;font-style:italic;">'
                'Ensemble models: Prophet, Holt-Winters, Linear Regression and baseline trend.'
                '</div></div>')
        ...
        gr.HTML('<div style="font-size:18px;font-weight:700;letter-spacing:0;text-transform:none;'
                'color:#0B1F3A;padding:12px 0 6px;border-top:3px solid #1B4F8A;margin-top:8px">'
                '📦 Operational Performance &amp; AI Business Insights</div>')
        ...
        pdf_file_output  = gr.HTML("", visible=False, elem_id="pdf-file-output")
    ...
    def confirm_step3(current_data, c1, c2, cert):
        ...
        if msme_from_step2 and cert is not None:
            # Gradio 4.x passes file as dict {'name': path} or tempfile object
            if isinstance(cert, dict):
                cert_path = cert.get('name') or cert.get('path') or str(cert)
            elif hasattr(cert, 'name'):
                cert_path = cert.name
            else:
                cert_path = str(cert)
            cert_text = ""
            # Primary: pdfplumber (in requirements.txt)
            try:
                import pdfplumber
                ...
                pass
            # Fallback: pypdf (lightweight, no C deps)
    ...
    def analyze_data(user_data, consent, file, lang='en'):
        ...
        def _fail(msg):
            # Returns all 42 values — stay on step5 so user sees the error message
            _no_nav = update_visibility_all('step5')   # keep step5 visible
            _hdrv   = _hdr(False)
            return (
                # Step 5 updates (19: +user_data_state unchanged on failure)
                f'<div style="color:#c0392b;padding:12px;background:#FFF0F0;border-radius:8px;'
                f'border:1px solid #e74c3c;font-size:13px">{msg}</div>',
                gr.update(visible=False),              # view_dashboard_btn hidden
                "", "", "", "", "",                    # kpi1-kpi5
                None, None, None, None,                # chart1-chart4
                gr.update(value="", visible=False),    # sum1
                gr.update(value="", visible=False),    # sum2
                gr.update(value="", visible=False),    # sum3
                gr.update(value="", visible=False),    # sum4
                gr.update(value="", visible=False),    # upload_message
                empty_dash,                            # dashboard_data_state
                None,                                  # df_state
                user_data,                             # user_data_state (unchanged on failure)
                # Navigation: stay on step5 (23)
                5,                                     # step_state
                *_no_nav,                              # step0_col..step7_col
                "",                                    # kpi_table_dash
                None, None, None, None,                # chart1_dash..chart4_dash
                gr.update(value="", visible=False),    # chart1_summary
                gr.update(value="", visible=False),    # chart2_summary
                gr.update(value="", visible=False),    # chart3_summary
                gr.update(value="", visible=False),    # chart4_summary
                None,                                  # granular_forecast_data_state
                *_hdrv,                                # logo_html, msme_hdr_btn, gov_hdr_btn
                gr.update(value=""), None, gr.update(visible=False),  # catalogue html/dl/col
            )
        ...
        # Safe defaults — ensure catalogue vars always exist even if generation fails
        _cat_list, _cat_html, _cat_csv = [], "", None
        try:
            ...
            # ── Data safety layer: clean messy values before analytics ────────
            df = _clean_raw_df(df)
            ...
            # ── Pillar 2: AI product categorisation ─────────────────────────────
            _p2_sku_col = next((c for c in df.columns if c in ("SKU_Name","Product_Name","product_name","Item_Name")), None)
            _p2_cat_col = next((c for c in df.columns if c in ("Product_Category","Category","category")), None)
            if _p2_sku_col:
                try:
                    df = classify_products_batch(df, sku_col=_p2_sku_col, cat_col=_p2_cat_col, use_llm=True)
                except Exception:
                    pass
            ...
            # ── Pillar 2: Append ONDC category summary panel to insights_html ───
            if "ONDC_Category" in df.columns:
                try:
                    _cat_counts = df["ONDC_Category"].value_counts()
                    _max_cnt = max(_cat_counts.max(), 1)
                    _cat_rows = ""
                    for _cat, _cnt in _cat_counts.items():
                        _bar_w = int(_cnt / _max_cnt * 100)
                        _cat_rows += (
                            "<tr>"
                            f"<td style='padding:6px 12px;font-size:12px;color:#0B1F3A;'>{_cat}</td>"
                            f"<td style='padding:6px 12px;font-size:12px;text-align:center;color:#1B4F8A;font-weight:700;'>{_cnt}</td>"
                            f"<td style='padding:6px 12px;'>"
                            f"<div style='width:{_bar_w}%;height:6px;background:#1B4F8A;border-radius:3px;'></div>"
                            f"</td></tr>"
                        )
                    _cat_summary_html = (
                        "<div style='margin-top:16px;border:1px solid #C8DCEF;border-radius:10px;overflow:hidden;'>"
                        "<div style='background:#0B1F3A;padding:10px 16px;'>"
                        "<span style='color:#A8D8FF;font-size:11px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;'>"
                        "📦 AI Product Categorisation (ONDC Taxonomy)"
                        "</span></div>"
                        "<table style='width:100%;border-collapse:collapse;background:#fff;'>"
                        "<thead><tr style='background:#EEF4FB;'>"
                        "<th style='padding:6px 12px;text-align:left;font-size:11px;color:#0B1F3A;'>ONDC Category</th>"
                        "<th style='padding:6px 12px;text-align:center;font-size:11px;color:#0B1F3A;'>SKUs</th>"
                        "<th style='padding:6px 12px;text-align:left;font-size:11px;color:#0B1F3A;'>Share</th>"
                        "</tr></thead>"
                        f"<tbody>{_cat_rows}</tbody></table>"
                        "<div style='padding:7px 12px;font-size:11px;color:#4A6A8A;border-top:1px solid #EEF4FB;'>"
                        "Categories auto-classified using Claude AI + ONDC L1 taxonomy · "
                        f"{len(_cat_counts)} distinct categories detected"
                        "</div></div>"
                    )
                    insights_html = _cat_summary_html + (insights_html or "")
                except Exception:
                    pass
            ...
            # ── Pillar 3B: Generate ONDC catalogue from scored data ──────────────
            _cat_list, _cat_html = generate_ondc_catalogue(df.copy(), user_data, use_llm=True)
            _cat_csv = export_catalogue_csv(_cat_list) if _cat_list else None
            s1,s2,s3,s4 = result[9],result[10],result[11],result[12]
            raw_df = result[15]
            # Run granular forecast in a thread with a hard 30s timeout.
            # Prevents any slow model from blocking the entire analyze_data call.
            import threading as _thr
            _gf_result = [None]
            def _run_gf():
                try: _gf_result[0] = generate_granular_forecast(df.copy())
                except Exception: pass
            _gf_thread = _thr.Thread(target=_run_gf, daemon=True)
            _gf_thread.start()
            _gf_thread.join(timeout=30)   # max 30s — if still running, skip
            gf = _gf_result[0]            # None if timed out
            ...
            try:
                ...
                _rpc = next((c for c in (
                    'replacement_count', 'Replacement_Units',
                    'replacement_units', 'units_replaced',
                    'Units_Replaced', 'Replacements', 'replacements'
                ) if c in _snap_df.columns), None)
            ...
            # ── Stay on step5, show insights + "View Dashboard →" button ─────
            # Correct flow: user reads AI insights on step5, then clicks the button
            # to go to step6 ONDC dashboard. Do NOT auto-navigate — that skips insights.
            _s1 = gr.update(value=s1, visible=bool(s1)) if s1 else gr.update(value="", visible=False)
            _s2 = gr.update(value=s2, visible=bool(s2)) if s2 else gr.update(value="", visible=False)
            _s3 = gr.update(value=s3, visible=bool(s3)) if s3 else gr.update(value="", visible=False)
            _s4 = gr.update(value=s4, visible=bool(s4)) if s4 else gr.update(value="", visible=False)
            _vis_5  = update_visibility_all('step5')   # stay on step5
            _hdrv   = _hdr(False)

            # ── Enrich user_data with dataset-derived business name ────────────
            # Now that the dataset is loaded, override company_name with the name
            # read directly from the data (MSME_ID lookup or Business_Name col).
            # This makes Steps 5/6/7 show the dataset's actual business name.
            _dataset_name = _extract_business_name(df, user_data)
            user_data = {**user_data, 'company_name': _dataset_name}

            return (
                # ── Step 5 component updates (19: +user_data_state) ───────
                insights_html or "✅ Analysis completed",  # insights_output
                gr.update(visible=True),                   # view_dashboard_btn visible
                k1, "", "", "", "",                        # kpi1-kpi5
                f1, f2, f3, f4,                            # chart1-chart4
                _s1, _s2, _s3, _s4,                        # sum1-sum4
                gr.update(value="", visible=False),        # upload_message
                dash,                                      # dashboard_data_state
                df_for_gov,                                # df_state
                user_data,                                 # user_data_state (enriched with dataset name)
                # ── Stay on step5 (23 nav values) ─────────────────────────
                5,                                         # step_state
                *_vis_5,                                   # step0_col..step7_col (9)
                "",                                        # kpi_table_dash (not populated yet)
                None, None, None, None,                    # chart_dashes (populated on button click)
                gr.update(value="", visible=False),        # chart1_summary
                gr.update(value="", visible=False),        # chart2_summary
                gr.update(value="", visible=False),        # chart3_summary
                gr.update(value="", visible=False),        # chart4_summary
                gf,                                        # granular_forecast_data_state
                *_hdrv,                                    # logo_html, msme_hdr_btn, gov_hdr_btn
                # ── Pillar 3B: ONDC catalogue outputs ────────────────────────────
                gr.update(value=_cat_html if _cat_html else ""),   # ondc_catalogue_html
                gr.update(value=_cat_csv if _cat_csv else "", visible=bool(_cat_csv)), # ondc_catalogue_dl
                gr.update(visible=bool(_cat_list)),                 # ondc_cat_col
            )
        except Exception as e:
            import traceback
    ...
    def _build_step7_data(df_raw, store_sel, cat_sel, prod_sel):
        ...
        # ── Fulfillment data detection ────────────────────────────────────────
        _FULFIL_SIGNALS = {
            'fulfillment_rate', 'fulfillment_rate', 'target_achievement_pct',
            'order_fulfilled', 'fulfilled_orders', 'orders_fulfilled',
            'delivery_status', 'delivery_time', 'order_status',
            'sla_met', 'orders_delivered', 'fulfil_rate',
        }
        _df_cols_lower_s7 = {c.lower() for c in df.columns}
        fulfillment_data_available = bool(tac) or bool(_df_cols_lower_s7 & _FULFIL_SIGNALS)
        ...
        # Fulfillment card: show N/A when no fulfillment data in dataset
        _fulfil_val   = f"{avg_fulfil:.1f}%" if fulfillment_data_available else "N/A"
        _fulfil_color = tgt_c               if fulfillment_data_available else "#9E9E9E"
        _fulfil_sub   = ("Target ≥85%" if fulfillment_data_available
                         else "N/A — Fulfillment data not available in uploaded dataset.")
        _fulfil_sub_c = tgt_c if fulfillment_data_available else "#BDBDBD"

        kpi_html = (
            '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px">'
            + _kcard("Total Units Sold",         f"{total_units:,}",    "#1B4F8A")
            + _kcard("Total Business Revenue",   _inr(total_sales),     "#1B4F8A")
            + _kcard("Average Profit Margin (%)",         f"{avg_margin:.1f}%",  mrg_c)
            + _kcard("Product Return Rate",      f"{avg_ret:.1f}%",     ret_c,
                     sub="Benchmark ≤7%", sub_color=ret_c)
            + _kcard("Total Replacement Units",  f"{total_repl:,}",     "#8b5cf6")
            + _kcard("Order Fulfilment Rate",    _fulfil_val,           _fulfil_color,
                     sub=_fulfil_sub, sub_color=_fulfil_sub_c)
            + '</div>'
        )
...
<div style="margin-bottom:16px">
  <div style="background:#EAF4FF;border:1px solid #C8DCEF;border-bottom:none;
              border-radius:8px 8px 0 0;padding:9px 16px 7px;color:#1B4F8A">
    <div style="font-size:11px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;">
      Top Selling Products by Revenue
    </div>
    <div style="font-size:10px;font-weight:400;letter-spacing:0;text-transform:none;
                color:#4A6A8A;margin-top:3px;">
      Products ranked by total revenue based on the currently selected Store, Category, and Product filters.
    </div>
  </div>
  <table style="width:100%;border-collapse:collapse;font-size:12.5px;
    ...
    <thead>
      <tr style="background:#0B1F3A">
        ...
        <th style="padding:9px 14px;color:#A8D8FF;font-weight:600;text-align:center">Product Return Rate</th>
      </tr>
    </thead>
    <tbody>{rows}</tbody>
  </table>
        ...
        def _style_ax(ax, title, ylabel="", fontsize=14):
            ax.set_title(title, fontsize=fontsize, fontweight='bold', pad=10,
                         color='#1A2D45', loc='left')
            if ylabel:
                ax.set_ylabel(ylabel, fontsize=12, color='#4A6A8A', labelpad=6)
            ax.tick_params(axis='both', labelsize=10, colors='#4A6A8A', length=3)
        ...
        if dc and (catc or is_product_drill) and sc:
            ...
            if is_product_drill and sc:
                ...
                ax1.set_xticklabels(all_months[::step1], rotation=45, ha='right', fontsize=11)
                ax1.legend(fontsize=11, ncol=3, loc='upper left',
                           framealpha=0.9, edgecolor='#D0E4F4', labelspacing=0.3)
                ax1.yaxis.set_major_formatter(mticker.FuncFormatter(_inr_ax))
                _style_ax(ax1, 'Category Revenue Trend (36 Months)', 'Sales (₹ Lakhs)')
        else:
            ax1.text(0.5, 0.5, 'No time-series data', ha='center', va='center',
                     transform=ax1.transAxes, fontsize=10, color='#7A92AA')
            _style_ax(ax1, 'Category Revenue Trend', 'Sales (₹ Lakhs)')
        ...
        if dc and (pmc or is_product_drill):
            ...
            if is_product_drill:
                ...
                if grp2 is not None and not grp2.empty:
                    ...
                    ax2.set_xticklabels(all_m2[::step2], rotation=45, ha='right', fontsize=11)
                    ax2.legend(fontsize=11, ncol=3, loc='upper left',
                               framealpha=0.9, edgecolor='#D0E4F4', labelspacing=0.3)
                    ax2.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f'{x:.0f}%'))
                _style_ax(ax2, 'Category Margin Performance (36 Months)', 'Margin %')
        else:
            ax2.text(0.5, 0.5, 'No margin data', ha='center', va='center',
                     transform=ax2.transAxes, fontsize=10, color='#7A92AA')
            _style_ax(ax2, 'Category Margin Performance', 'Margin %')
        ...
        def _forecast(monthly_df_or_series, n):
            ...
            # ── Candidate A: Prophet — SKIPPED in step7 charts ───────────────
            # Prophet (3-8s/fit × 2 fits × 2+ calls) causes 20-30s hang.
            # HW + LR + Baseline are fast (<0.1s total) and accurate for drill-down charts.
            prophet_fc_full = None  # not used in step7 path
        ...
        _style_ax(ax3, f'6-Month Sales Forecast', 'Sales (₹L)')
        ...
        _style_ax(ax4, f'12-Month Sales Projection', 'Sales (₹L)')
        ...
        if dc and tac and fulfillment_data_available:
            df5 = df.copy()
            ...
            if not ft.empty:
                ...
                ax5.text(0.5, 0.5, 'No fulfilment data available for selected filters.', ha='center', va='center',
                         transform=ax5.transAxes, fontsize=10, color='#7A92AA')
        else:
            # ── Styled placeholder when fulfillment data is not in the dataset ──
            ax5.set_facecolor('#FAFAFA')
            for sp in ax5.spines.values(): sp.set_color('#E8E8E8')
            ax5.text(0.5, 0.62,
                     'No fulfilment data available in the uploaded dataset.\nThis chart activates when delivery or fulfilment columns are detected.',
                     ha='center', va='center', transform=ax5.transAxes,
                     fontsize=10, color='#9E9E9E', style='italic', linespacing=1.6)
            ax5.text(0.5, 0.36,
                     'Add a Fulfillment_Rate, Delivery_Status, Order_Status,\nor SLA_Met column to your dataset to enable this chart.',
                     ha='center', va='center', transform=ax5.transAxes,
                     fontsize=7.5, color='#BDBDBD', linespacing=1.55)
            ax5.set_xticks([]); ax5.set_yticks([])
        ylabel5 = 'Fulfillment Rate %' if using_fulfilment_rate else 'Target Achievement %'
        _style_ax(ax5, 'Product Fulfilment Performance', ylabel5 if fulfillment_data_available else 'Fulfilment Rate %')
        ...
        _style_ax(ax6, 'Product Sales vs Product Returns', 'Sales (₹L)')
        ...
        if is_product_drill and dc and ivc:
            ...
            try:
                ...
                pass
            _style_ax(ax7, 'Inventory Health & Reorder Risk', '')
        ...
        margin_action_li = (
            ...
            f'Current average margin: <strong>{avg_margin:.1f}%</strong> — '
            f'{margin_gap}pp below the 20% target. A 5% cost reduction could unlock ~{_inr(total_sales * 0.05 if total_sales else 0)} in additional profit.</li>'
            if margin_gap > 0 else
            f'<li>✅ <strong>Inventory Health: Margin is strong at {avg_margin:.1f}%</strong>. '
            f'Consider expanding your <em>{top_cat}</em> SKU catalogue to capture additional revenue.</li>'
        )
        # Inventory action
        inv_action_li = (
            f'<li>📦 <strong>Inventory Recommendation: Increase restocking by ~{inv_action_pct}% for <em>{top_cat}</em></strong>. '
            f'Current order fulfilment rate is <strong>{avg_fulfil:.1f}%</strong> '
            f'(target ≥85%). Stockout risk is elevated — pre-order inventory for the next 30 days.</li>'
            if inv_action_pct > 0 else
            f'<li>✅ <strong>Inventory Health: Inventory levels are healthy</strong>. '
            f'Order fulfilment rate: {avg_fulfil:.1f}%. '
            f'Focus on optimising reorder cycle frequency to reduce holding costs.</li>'
        )
        # Returns action
        ret_action_li = (
            f'<li>🔁 <strong>Quality Performance: Product return rate is {avg_ret:.1f}% — above the 7% threshold for <em>{top_cat}</em></strong>. '
            f'Review the top 3 returned SKUs and introduce pre-dispatch quality checks to reduce returns.</li>'
            if ret_action else
            f'<li>✅ <strong>Quality Performance: Product return rate is {avg_ret:.1f}% — within the acceptable retail benchmark (&lt;7%)</strong>. '
            f'Current quality controls are effective; continue monitoring SKU-level return trends.</li>'
        )
        # Forecast growth action
        if fc_pct > 3:
            fc_action_li = (
                f'<li>📈 <strong>Demand Outlook: Demand expected to increase by {fc_pct}–{fc_pct+4}% over the next 6 months — <em>{top_cat}</em></strong>. '
                f'Projected revenue: <strong>{_inr(total_6m) if total_6m else "—"}</strong>. '
                f'Align procurement and staffing 3 weeks ahead of the forecast peak.</li>'
            )
        elif fc_pct < -3:
            fc_action_li = (
                f'<li>⚠️ <strong>Demand Outlook: A {abs(fc_pct)}% demand slowdown is projected over the next 6 months — <em>{top_cat}</em></strong>. '
                f'Projected revenue: <strong>{_inr(total_6m) if total_6m else "—"}</strong>. '
                f'Consider reducing your next procurement order to avoid overstock.</li>'
            )
        else:
            fc_action_li = (
                f'<li>📊 <strong>Demand Outlook: Demand is projected to remain stable — <em>{top_cat}</em></strong>. '
...
<div style="background:linear-gradient(160deg,#EAF4FF 0%,#F8FAFF 100%);
  ...
  <div style="background:#fff;border-radius:8px;padding:10px 13px;
    ...
    &nbsp;|&nbsp;
    <span style="font-weight:700;color:#1B4F8A">Product Return Rate:</span>
    <span style="color:{ret_c};font-weight:700">{avg_ret:.1f}%</span><br>
    <span style="font-weight:700;color:#1B4F8A">Order Fulfilment:</span>
    <span style="color:{tgt_c};font-weight:700">{avg_fulfil:.1f}%</span>
    &nbsp;|&nbsp;
    <span style="font-weight:700;color:#1B4F8A">6-Month Forecast:</span>
    <span style="color:#1B4F8A;font-weight:700">{_inr(total_6m) if total_6m else "—"}</span>
  </div>
</div>"""

        # ── AI Decision Recommendations ─────────────────────────────────────────
        # Built from already-computed variables only — no new calculations

        # 1. Inventory Planning
        if fc_pct > 3:
            _reco_inv = (f'Demand is projected to grow by <strong>{fc_pct}%–{fc_pct+4}%</strong> over '
                         f'the next 6–12 months. Increase procurement planning for high-demand SKUs '
                         f'such as <em>{top_cat}</em> to prevent stock-outs and support sales growth.')
        elif inv_action_pct > 0:
            _reco_inv = ('Inventory turnover signals indicate potential stock pressure. '
                         'Consider pre-order planning or supplier lead-time optimisation '
                         'to avoid fulfilment delays and lost sales.')
        else:
            _reco_inv = (f'Inventory levels are currently healthy with an order fulfilment rate of '
                         f'<strong>{avg_fulfil:.1f}%</strong>. '
                         f'Maintain the current procurement cycle with a 10% safety buffer '
                         f'to sustain operational stability.')

        # 2. Product Expansion
        _reco_prod = (f'High-performing SKUs in the <em>{top_cat}</em> category demonstrate '
                      f'strong sales momentum with a projected 6-month revenue of '
                      f'<strong>{_inr(total_6m) if total_6m else "—"}</strong>. '
                      f'Expanding the product catalogue within similar categories may '
                      f'further accelerate revenue growth.')

        # 3. Pricing & Profitability
        if margin_gap > 0:
            _reco_price = (f'Current profit margin of <strong>{avg_margin:.1f}%</strong> is '
                           f'{margin_gap:.1f}pp below the 20% target. Renegotiate supplier costs '
                           f'or review pricing on low-margin SKUs before launching '
                           f'promotional campaigns to protect profitability.')
        else:
            _reco_price = (f'Average profit margin of <strong>{avg_margin:.1f}%</strong> provides '
                           f'strong headroom for competitive pricing strategies. '
                           f'Consider promotional pricing or bundled offers on top-selling SKUs '
                           f'to increase market share without significantly impacting profitability.')

        # 4. Quality & Returns
        if ret_action:
            _reco_qual = (f'Product return rate of <strong>{avg_ret:.1f}%</strong> exceeds the 7% '
                          f'operational benchmark. Review supplier quality checks, packaging '
                          f'standards, and top returned SKUs to reduce return-related costs '
                          f'and protect customer satisfaction.')
        else:
            _reco_qual = (f'Product return rate of <strong>{avg_ret:.1f}%</strong> remains within '
                          f'the healthy operational benchmark (&lt;7%), indicating strong product '
                          f'quality and fulfilment standards. Maintain existing quality assurance '
                          f'processes to sustain customer satisfaction.')

        # 5. Marketplace Growth
        _reco_mkt = ('Marketplace channel performance indicates strong digital sales potential. '
                     'Expanding additional SKUs through ONDC Seller Network Participants '
                     'can improve marketplace visibility and generate incremental revenue '
                     'beyond the current offline retail baseline.')

        def _reco_item(icon, title, text):
            return (
                f"<div style='display:flex;gap:12px;padding:12px 14px;border-radius:8px;"
                f"background:#fff;border:1px solid #C8DCEF;margin-bottom:8px;'>"
                f"<div style='font-size:20px;flex-shrink:0;line-height:1.4;'>{icon}</div>"
                f"<div>"
                f"<div style='font-size:12px;font-weight:700;color:#0B1F3A;margin-bottom:3px;'>{title}</div>"
                f"<div style='font-size:12px;color:#334155;line-height:1.6;'>{text}</div>"
                f"</div></div>"
            )

        ai_reco_html = f"""
<div style='margin-top:24px;border:1px solid #C8DCEF;border-radius:12px;overflow:hidden;'>
  <div style='background:#0B1F3A;padding:14px 18px;'>
    <div style='font-size:13px;font-weight:800;color:#FFFFFF;letter-spacing:0.3px;'>🤖 AI Decision Recommendations</div>
    <div style='font-size:11px;color:#A8D8FF;margin-top:3px;font-weight:300;'>
      AI-generated strategic guidance based on sales trends, demand forecasts, return patterns, and inventory signals.
    </div>
  </div>
  <div style='padding:14px 16px;background:#F8FAFF;'>
    {_reco_item('📦', 'Inventory Planning', _reco_inv)}
    {_reco_item('🚀', 'Product Expansion Strategy', _reco_prod)}
    {_reco_item('💰', 'Pricing & Profitability Strategy', _reco_price)}
    {_reco_item('🔁', 'Quality & Returns Management', _reco_qual)}
    {_reco_item('🌐', 'Marketplace Growth Opportunity', _reco_mkt)}
  </div>
</div>"""

        # Append recommendations to AI summary
        ai_html = ai_html + ai_reco_html
    ...
    fetch_btn.click(
        fn=lambda: gr.update(value="⏳ Fetching MSME data..."),
        inputs=[], outputs=[fetch_status]
    ).then(
        fn=_fetch_msme_data, inputs=[msme_number_input],
        outputs=[fetched_name, fetched_org, fetched_activity, fetched_type,
                 fetched_state, fetched_city, fetched_industry, fetch_status]
    )

    # ── Pillar 3A: UDYAM live format verification (fires on every keystroke) ──
    msme_number_input.change(
        fn=lambda v: render_udyam_verify_html(verify_udyam_number(v)),
        inputs=[msme_number_input],
        outputs=[udyam_verify_html]
    )
    ...
    analyze_btn.click(
        ...
        # analyze_data now returns all 41 outputs in one shot:
        #   18 step5 component updates + 23 step6 navigation signals.
        # This bypasses the Gradio .then() State-not-readable-mid-chain bug
        # where dashboard_data_state written in step 2 was not readable in step 4.
        analyze_data, [user_data_state, consent_check, file_upload, lang_state],
        [insights_output, view_dashboard_btn,
         kpi1, kpi2, kpi3, kpi4, kpi5,
         chart1, chart2, chart3, chart4,
         sum1, sum2, sum3, sum4,
         upload_message, dashboard_data_state, df_state,
         user_data_state,
         ...
         ondc_catalogue_html, ondc_catalogue_dl, ondc_cat_col]
    ).then(
    ...
    def _generate_pdf(user_data, dashboard_data, df_raw, granular_data):
        ...
        try:
            ...
            if path and os.path.exists(path):
                ...
                with open(path, "rb") as _fpdf:
                    _b64 = _b64pdf.b64encode(_fpdf.read()).decode()
                _fname = os.path.basename(path)
                _dl = (
                    "<div style='padding:10px 14px;background:#f0fdf4;border:1px solid #bbf7d0;"
                    "border-radius:8px;margin-top:8px;display:flex;align-items:center;gap:10px;'>"
                    "<span style='font-size:16px;'>📥</span>"
                    "<a href='data:application/pdf;base64," + _b64 + "' download='" + _fname + "' "
                    "style='font-size:13px;font-weight:600;color:#166534;text-decoration:none;"
                    "border-bottom:1px solid #166534;'>Click here to download PDF Report</a>"
                    "</div>"
                )
                return gr.update(value=_dl, visible=True)
            return gr.update(value="<div style='color:#dc2626;font-size:12px;'>"
                             "⚠️ PDF could not be generated</div>", visible=True)
    ...
    def switch_lang_en():
        return ('en', gr.update(variant='primary'), gr.update(variant='secondary'), gr.update(value='**Active: English 🇬🇧**'),
                ...
                gr.update(value="🚀 Continue to AI Forecast & Product Intelligence →"),
...
if __name__ == "__main__":
    ...
    #
    # Gradio 4.20+ removed concurrency_count and api_open from queue().
    # Introspect the signature so this works across all Gradio 4.x versions.
    import inspect as _inspect_q
    _queue_sig = set(_inspect_q.signature(demo.queue).parameters.keys())
    _queue_kwargs = {"max_size": 20, "status_update_rate": 0.5}
    if "concurrency_count" in _queue_sig:
        _queue_kwargs["concurrency_count"] = 2
    if "api_open" in _queue_sig:
